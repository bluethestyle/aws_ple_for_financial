...
> **합의 메커니즘 정본 참조**: 합의 프레이밍의 권위 있는 설계 근거는 `docs/design/13_consensus_qec_framing.md`를 따른다. 본 절은 운영/감사 맥락에서의 적용만 기술한다.

AWS와 온프렘에서 합의 메커니즘의 **방식 자체가 다르다**:

**AWS: 3-에이전트 합의 (모듈은 병렬 지원, 프로덕션은 순차 실행)**
- Sonnet 3개, 서로의 출력을 보지 않는 독립 투표 + 관점 변주: α 보수적 / β 통계적 / γ 비즈니스
- `ConsensusArbiter`는 `parallel=True/False`를 모두 지원하지만, **프로덕션 아비터는 순차로 고정**된다 — `pipeline_reports.py`의 `_build_ops_arbiter`/`_build_audit_arbiter`가 `config={"agents":3, "parallel":False}`로 생성. 병렬은 어디까지나 모듈 capability이며 프로덕션에서는 순차로 override된다.
- 소요시간은 순차 실행 기준 3회 호출 합산(병렬이면 ~5초 수준이나 프로덕션 기본값 아님), 비용 3×
...
|---|---|---|---|---|---|
| AWS | Sonnet | 3 (프로덕션 순차) | -- | 3 | 순차 3-call 합산 (병렬 시 ~5초이나 프로덕션 기본 아님) |
...
| 분류 | AWS (3명, 순차 투표) | 온프렘 (5명) | 처리 |
|---|---|---|---|
| **Consensus** (만장일치) | 3/3 일치 | 5/5 일치 | 통과 또는 확정 WARN |
| **최우선 리뷰** (다수) | 2/3 이상 | 3/5 이상 | 즉시 담당자 리뷰 |
| **마이너리티 리포트** (소수) | 1/3 이상 | 1~2/5 이상 | 2순위 리뷰 리스트 |

> 분류 규칙은 투표 집계 기준이며 실행 순서(병렬/순차)와 무관하다. AWS 프로덕션 아비터는 순차로 3표를 모은다. 정본은 `docs/design/13_consensus_qec_framing.md`.
...
| # | 에이전트 | 점검 항목 | 판정 |
|---|---|---|---|
| 3.1 | Ops | 전 태스크 val metric 수렴 (최근 3 epoch 변동 < 1%) — binary: AUC, multiclass: F1-macro, regression: MAE (태스크 유형별 지표 집계, 12개 태스크) | PASS/WARN |
...
ops_report:
  ...
  all_checkpoints:
    ...
    CP3: {status: YELLOW, detail: "1/12 tasks degrading"}
...
### 4.2 신규 컴포넌트 — As-Built 매핑 (구현 완료)

아래 컴포넌트는 모두 **커밋되어 코드베이스에 존재**한다. `OpsReporter`/`AuditReporter`는 `core/agent/pipeline_reports.py`의 `run_pipeline_reports()`에 배선되어 있고, 각각 3-에이전트 `ConsensusArbiter` 패널(`_build_ops_arbiter`/`_build_audit_arbiter`)이 뒷받침한다.

| 컴포넌트 | 에이전트 | 설명 | 파일 경로 |
|---|---|---|---|
| `OpsCollector` | Ops | 7개 체크포인트 측정값 수집기 | `core/agent/ops/collector.py` |
| `OpsDiagnoser` | Ops | 연쇄 영향 분석 (cross-checkpoint) | `core/agent/ops/diagnoser.py` |
| `OpsReporter` | Ops | 리포트 생성 + 전달 (Slack/Email/SNS) | `core/agent/ops/reporter.py` |
| `CloudWatchLogAnalyzer` | Ops | 서빙 로그 패턴 분석 (latency/에러 추세) | `core/agent/ops/cloudwatch_log_analyzer.py` |
| `StratifiedReasonSampler` | Audit | Tier 2 층화추출 + 우선순위 샘플링 | `core/agent/audit/reason_sampler.py` |
| `GroundingValidator` | Audit | 사유 텍스트 ↔ IG top-K 일치율 검증 | `core/agent/audit/grounding_validator.py` |
| `Tier1Aggregator` | Audit | SelfChecker 결과 추이 집계 (Tier 1) | `core/agent/audit/tier1_aggregator.py` |
| `IntersectionalFairnessAnalyzer` | Audit | 교차 보호속성 분석 | `core/agent/audit/intersectional_fairness.py` |
| `BiasStageAttributor` | Audit | 편향 발생/증폭 단계 분리 | `core/agent/audit/bias_stage_attributor.py` |
| `AuditDiagnoser` | Audit | focus_areas 생성 + 우선순위 판단 | `core/agent/audit/diagnoser.py` |
| `AuditReporter` | Audit | 리포트 생성 + 전달 | `core/agent/audit/reporter.py` |
| `AgentEventBridge` | 양쪽 | 상호 트리거 + GovernanceReport 연동 (모듈 구현) | `core/agent/event_bridge.py` |
| `DiagnosticCaseStore` | 양쪽 | LanceDB 기반 진단 케이스 저장/검색/통계 | `core/agent/case_store.py` |
| `ConsensusArbiter` | 양쪽 | 3-에이전트 합의 판정 + 마이너리티 분류 (프로덕션 순차) | `core/agent/consensus.py` |
| `ChangeDetector` | 양쪽 | 변경 감지 Push/Pull 채널 + 표준 이벤트 포맷 | `core/agent/change_detector.py` |
| `ToolRegistry` | 양쪽 | 도구 정의(JSON Schema) 관리 + 호출 메커니즘 | `core/agent/tool_registry.py`, `core/agent/tool_wrappers.py` |
| `SendNotification` (구현 클래스 `NotificationService`) | 양쪽 | Slack/Email/SNS 리포트 전달 | `core/agent/notification.py` |
| `BaseAgent` | 양쪽 | 에이전트 공통 베이스 + 체크리스트 루프 | `core/agent/base.py` |
| `BedrockDialogSession` | 양쪽 | Tool Use 대화 인터페이스 | `core/agent/bedrock_dialog.py` |
| `BudgetTracker` | 양쪽 | LLM 호출 예산 추적 (소프트 경고/하드 정지) | `core/agent/budget.py` |
| `HeartbeatScheduler` | Ops | 주기 헬스 점검 (PaperClip Heartbeat) | `core/agent/heartbeat.py` |
| `ToolTracer` | 양쪽 | 도구 호출 전후 자동 로깅 (Full Trace) | `core/agent/tracer.py` |

> **배선 상태**: `run_pipeline_reports()`가 `enable_consensus=True`일 때 `OpsReporter`/`AuditReporter`를 각각 stakeholder 패널(`_build_ops_arbiter`: SRE/MLOps/Biz, `_build_audit_arbiter`: Regulator/Risk/AuditTrail)과 함께 인스턴스화한다. Bedrock provider 부재 시 룰 엔진 리포트로 graceful degradation.

#### 여전히 미구현(pending) — IaC/훅 영역

| 항목 | 상태 | 비고 |
|---|---|---|
| `AgentEventBridge`의 EventBridge/Lambda IaC 배포 | 미구현 | 모듈 로직은 존재하나 CloudFormation/Lambda 인프라 배선은 아직 없음 (§6 미결 과제) |
| `git post-commit` hook 인프라 | 미구현 | `ChangeDetector`는 구현됨. post-commit → ChangeDetector 전달 훅은 샘플 수준 (§6 미결 과제) |
...
이 절은 원래 구현 로드맵(단계별 LOC 추정)이었으나, 해당 컴포넌트는 모두 **커밋되어 코드베이스에 존재**한다. 아래는 설계 → 실제 파일 매핑이다.
...
| 컴포넌트 | 파일 경로 | 상태 |
|---|---|---|
| `ToolRegistry` + 래퍼 + Bedrock 내보내기 | `core/agent/tool_registry.py`, `core/agent/tool_wrappers.py` | 구현 완료 |
| `ChangeDetector` (Push/Pull 채널) | `core/agent/change_detector.py` | 구현 완료 (git post-commit 훅 IaC만 pending) |
| `BaseAgent` + 체크리스트 루프 | `core/agent/base.py` | 구현 완료 |

**추천사유 품질 Audit (AV3)**

| 컴포넌트 | 파일 경로 | 상태 |
|---|---|---|
| `StratifiedReasonSampler` — 27 스트라텀, 과표집 | `core/agent/audit/reason_sampler.py` | 구현 완료 |
| `GroundingValidator` — 사유↔IG top-K + 품질 점수 | `core/agent/audit/grounding_validator.py` | 구현 완료 |
| `Tier1Aggregator` — SelfChecker 추이 집계 | `core/agent/audit/tier1_aggregator.py` | 구현 완료 |

**편향 심화 Audit (AV1)**

| 컴포넌트 | 파일 경로 | 상태 |
|---|---|---|
| `IntersectionalFairnessAnalyzer` — 교차 보호속성 DI | `core/agent/audit/intersectional_fairness.py` | 구현 완료 |
| `BiasStageAttributor` — 단계별 DI + 증폭 식별 | `core/agent/audit/bias_stage_attributor.py` | 구현 완료 |
| `AuditDiagnoser` — focus_areas 생성 | `core/agent/audit/diagnoser.py` | 구현 완료 |
| `AuditReporter` — 리포트 생성 + 전달 | `core/agent/audit/reporter.py` | 구현 완료, `run_pipeline_reports()`에 배선 |

**운영 에이전트 + 합의**

| 컴포넌트 | 파일 경로 | 상태 |
|---|---|---|
| `OpsCollector` — 7 체크포인트 수집 | `core/agent/ops/collector.py` | 구현 완료 |
| `OpsDiagnoser` — 연쇄 영향 룰 테이블 | `core/agent/ops/diagnoser.py` | 구현 완료 |
| `OpsReporter` — 템플릿 리포트 | `core/agent/ops/reporter.py` | 구현 완료, `run_pipeline_reports()`에 배선 |
| `CloudWatchLogAnalyzer` — 서빙 로그 분석 | `core/agent/ops/cloudwatch_log_analyzer.py` | 구현 완료 |
| `ConsensusArbiter` — 3-에이전트 합의 + 마이너리티 | `core/agent/consensus.py` | 구현 완료. **프로덕션 순차**(`parallel=False`), 병렬은 모듈 capability |

**진단 케이스 스토어 + 메모리**

| 컴포넌트 | 파일 경로 | 상태 |
|---|---|---|
| `DiagnosticCaseStore` — LanceDB + 4 도구 | `core/agent/case_store.py` | 구현 완료 |
| `TemporalFactStore` — 시간적 사실 그래프 | `core/agent/temporal_fact_store.py` | 구현 완료 |
| `DialogRecallMemory` — 세션 간 대화 이력 | `core/agent/dialog_recall.py` | 구현 완료 |

**통합 + AWS 확장**

| 컴포넌트 | 파일 경로 | 상태 |
|---|---|---|
| `AgentEventBridge` — 상호 트리거 | `core/agent/event_bridge.py` | 모듈 구현 완료 (EventBridge/Lambda IaC만 pending) |
| `SendNotification` — Slack/Email/SNS | `core/agent/notification.py` | 구현 완료 |
| `BedrockDialogSession` — Tool Use 대화 | `core/agent/bedrock_dialog.py` | 구현 완료 |
| `BudgetTracker` — 예산 추적 | `core/agent/budget.py` | 구현 완료 |
| `HeartbeatScheduler` — 주기 헬스 점검 | `core/agent/heartbeat.py` | 구현 완료 |
| `ToolTracer` — Full Trace 로깅 | `core/agent/tracer.py` | 구현 완료 |
| 리포트 오케스트레이션 | `core/agent/pipeline_reports.py` (`run_pipeline_reports`) | 구현 완료, OpsReporter/AuditReporter + 합의 패널 배선 |

> **추천사유 4개 갭(GAP 1~4)**: 본 문서 후반 "추천사유 생성 파이프라인 연동" 절 참조. 해당 갭 수정은 추천사유 코드에 반영되었다.

### 디렉토리 구조 (As-Built)

```
core/agent/
    __init__.py, base.py, tool_registry.py, tool_wrappers.py,
    change_detector.py, consensus.py, case_store.py,
    temporal_fact_store.py, dialog_recall.py, event_bridge.py,
    notification.py, bedrock_dialog.py, budget.py, heartbeat.py,
    tracer.py, pipeline_reports.py
    ops/   collector.py, diagnoser.py, reporter.py, cloudwatch_log_analyzer.py
...
configs/financial/
    agent_tools.yaml   # 도구 스키마 (published 39 / 배선 43 callable)
```
...
| 항목 | 비고 |
|---|---|
| `AgentEventBridge`의 EventBridge/Lambda IaC | 모듈 로직은 존재, CloudFormation/Lambda 인프라 배선 미완 (§6 미결 과제) |
| `git post-commit` hook 인프라 | `ChangeDetector`는 구현됨, post-commit → ChangeDetector 전달 훅은 샘플 수준 (§6 미결 과제) |
...
> **Claude Sonnet 선택 이유**: Claude Sonnet은 Bedrock 네이티브로 Marketplace 온보딩 없이 즉시 사용 가능. 모델 ID는 config-resolved이며, 미지정 시 `pipeline_reports.py`의 `_CONSENSUS_FALLBACK_MODEL_ID = "global.anthropic.claude-sonnet-4-6"`로 fallback. `llm_provider.py` 팩토리 패턴으로 config 변경만으로 전환 가능.
