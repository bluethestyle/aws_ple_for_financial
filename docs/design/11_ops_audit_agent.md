...
| # | 에이전트 | 점검 항목 | 판정 |
|---|---|---|---|
| 3.1 | Ops | 전 태스크 val metric 수렴 (최근 3 epoch 변동 < 1%) — binary: AUC, multiclass: F1-macro, regression: MAE (태스크 유형별 지표 집계, 14개 태스크) | PASS/WARN |
...
| 6.4 | Audit | 국내 규제 20항목 critical failure 없음 | PASS/FAIL |
...
- 파이프라인 완료: `_PipelineState.mark_complete()` → 이벤트 발행
...
| AV4 | 규제 적합성 | `RegulatoryComplianceChecker` + `EUAIActMapper` + `FRIAEvaluator` | 20개 국내규제 + 17개 EU AI Act 항목 준수율 |
...
| 3-5 | LLM Provider 확장 — Solar, LocalLLM(Qwen/Exaone) | ~150 | 없음 | MED |
...
|---|---|---|---|---|
| L2a 사유 생성 | **Solar (Upstage)** | 한국어 특화, 금융 존댓말 톤 자연스러움 | 입 ~600 / 출 ~80 | ~$0.002 |
| L2b self-critique | **Solar (Upstage)** | 생성 모델 ≤ 크리틱 모델. 한국어 뉘앙스 판단 필요 | 입 ~800 / 출 ~100 | ~$0.003 |
...
| 기준 | Solar (Upstage) | Claude Haiku | Llama 3.1 |
|---|---|---|---|
| 한국어 자연스러움 | **최상** (한국어 특화) | 상 (범용) | 중하 (영어 중심) |
| 금융 존댓말 톤 | **최상** | 상 | 하 |
| 비용 | 저 | 저 | 최저 |
| Bedrock 가용성 | Marketplace (확인 필요) | 네이티브 | 네이티브 |

> **Solar 가용성**: `ap-northeast-2` 리전 확인 필요. 불가 시 Claude Haiku fallback. `llm_provider.py` 팩토리 패턴으로 config 변경만으로 전환 가능.

### L2a 비동기 처리 아키텍처

```
실시간 서빙                 비동기 L2a                   다음 서빙
L1 템플릿 즉시 반환    +    SQS → 워커 → Solar/Haiku    →    캐시에서 L2a 반환
...
| 모델 관리 | Solar+Haiku+Sonnet+Opus+Titan 5개 | `llm_provider.py` 팩토리로 config별 프로바이더 주입 |
