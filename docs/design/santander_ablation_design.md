...
> **데이터셋**: 941,132 users x 89 columns x 12 tasks x 7 shared experts x ~349D input / 403D after Phase 0 (benchmark v12)
> **최종 갱신**: 2026-04-13
...
Stage 5    레이블 파생        LabelDeriver: 12개 태스크 레이블 생성
...
raw_data["main"] (941K x 89)
  ...
  |
  +-- Stage 5 --> df_labels (941K x 12 label cols)
...
```
Tier 1 — Core (3 tasks)       직접 레이블. 데이터셋에서 바로 사용 가능
Tier 2 — Derived (1 task)     프로필/합성 컬럼에서 규칙으로 파생
Tier 3 — Product Group (5)    nba_label 인덱스에서 그룹별 이진 분류
Tier 5 — Txn NBA (3 tasks)    txn_mcc_seq 거래 시퀀스에서 파생
```

> **2026-04-12**: has_nba (binary, Tier 1) → nba_primary (multiclass) 통합. nba_primary class 0 = no NBA.
> **2026-05-01**: segment_prediction (multiclass, Tier 3) 제거 — 결정론적 leakage (commit 541a982).
> 총 12 tasks (binary 7개, multiclass 2개, regression 3개). 역사적 축소: 18→14→13→12.
...
| `nba_primary` | multiclass(25) | CE | 2.5 | 어떤 상품 추천? class 0=no NBA (has_nba 통합), class 1-24=24개 상품 |

> **참고**: has_nba는 2026-04-12에 nba_primary로 통합됨. nba_primary class 0 = no NBA (positive rate ~2.98%). CE 가중치는 class imbalance를 반영하여 조정.
...
> `segment_prediction` (multiclass(4), segment string_map) removed 2026-05-01 (commit 541a982) — deterministic leakage.
...
```
engagement  "고객이 반응하는가"    [next_mcc, top_mcc_shift]
            intra=0.8, inter=0.3
            # has_nba 제거됨 (2026-04-12): nba_primary로 통합

lifecycle   "고객이 어디에 있는가"  [churn_signal, product_stability]
            intra=0.7, inter=0.3
            # segment_prediction 제거됨 (2026-05-01, commit 541a982): 결정론적 leakage

value       "언제/어디서 가치를"    [mcc_diversity_trend, cross_sell_count]
            intra=0.6, inter=0.3

consumption "무엇을 소비하는가"    [nba_primary, will_acquire_*5]
            intra=0.7, inter=0.3
```
...
> has_nba→nba_primary 전이 제거됨 (2026-04-12): has_nba가 nba_primary에 통합되어 자기 전이가 불필요해짐.
...
|---|---|---|
| Binary | Focal Loss | alpha, gamma per task; 극단적 불균형 (will_acquire_deposits ~1%) 대응 |
...
**Note**: The v1 paper reports on the 23-scenario slice used for the canonical results (14 joint feature+expert + 9 structure cross variants on the fixed 13-task configuration — historical v1 paper task set). The 4-dimension × 9-structure grid (36 cells) is the full experimental design space; in the v1 paper the 9-structure cross was evaluated at the `tasks_13` row. The current full task set is **12** (`tasks_12`) after segment_prediction was removed 2026-05-01 (commit 541a982).
...
| `tasks_10` | 10 | + Tier 3 Product Group (일부) + Tier 5 나머지 |
| `tasks_12` | 12 | 전체 현행 셋 (Tier 3 완전 포함; segment_prediction 제거 2026-05-01, commit 541a982) |

> `tasks_13` 행은 v1 paper 기준(segment_prediction 포함) 역사적 라벨이며, 현행 full 셋은 `tasks_12`.
...
#### Track C: CGC Fusion Variant Family (residual-recovery) — 2026-06-26 커밋 (50b02b6)

CGC gate 위에 적용하는 fusion 변형 패밀리. 게이트 지점에서 **상호 배타(mutually exclusive)** 이며 cgc를 제외하면 전부 default-off.

| 변형 | `residual_method` | 설명 |
|---|---|---|
| `cgc` | — (default) | 기본 CGC gate-weighted 합산 |
| `residual_complement` (M1) | `complement` | (1−gate) 전체 complement residual 합산 |
| `residual_orthogonal` (OCP) | `orthogonal` | gate primary 방향을 각 expert 출력에서 사영 제거 후 직교 잔차만 회수 (직교여공간보정) |
| `eceb` | `eceb` | ECEB 변형 |

- OCP 활성화: `residual_recovery.enabled=true` + `residual_method='orthogonal'` (기본 `residual_recovery.enabled=false`이므로 명시 활성 필요).
- 구현: `core/model/ple/experts.py`. 3-way Spot ablation 런처: `scripts/run_sagemaker_ocp.py` (baseline cgc / M1 complement / OCP orthogonal).
- 측정 결과(CCA redundancy 포함)는 §7.2 Expert Redundancy CCA 및 `docs/paper3_restructure_plan.md` §7 참조.
...
- **배선 (2026-06-18; 커밋 50b02b6 2026-06-26)**: 이전엔 호출부 없는 dead module(config `expert_redundancy.enabled`만 존재)이었으나, 이제 `containers/training/train.py` eval에 config-gated(`analysis.expert_redundancy.enabled`, **default-TRUE**) best-effort 블록으로 배선되어 **모든 학습 job이 `expert_redundancy.json` 산출**. 독립 러너 `scripts/run_expert_redundancy.py`도 제공(체크포인트 → CCA).
- **첫 측정 (phase0_v14, seed 42, N=40960, 7 shared experts)**: baseline(cgc) mean ρ = **0.198** (21쌍 대부분 LOW, perslay·optimal_transport는 거의 모든 쌍 0.000) → expert 표현방향이 **이미 직교**. OCP(`residual_orthogonal`, gate primary 방향 사영 제거 후 잔차 회수) 적용 시 ρ를 −0.0055로 최대 직교화하나 헤드룸이 작아 task metric 이득은 미미. 상세 [[project-ocp-experiment]] / `docs/paper3_restructure_plan.md` §7.
...
scoring:
  ...
  weights:
    ...
    cross_sell_count: 0.25  # 교차판매 기회
    churn_signal: 0.20  # 이탈 위험 (역수)
    product_stability: 0.15  # 안정성
```
...
## 부록 A: 피처 그룹 상세 (12개 그룹, ~500D — benchmark-v12 라우팅 뷰)

> **차원 기준 주의**: 본 부록의 12개 그룹 / ~500D는 **benchmark-v12 라우팅 뷰**(이 ablation 문서가 쓰는 피처 파이프라인 기준)다. 현행 canonical santander 피처 사양은 **1211D / 17 santander feature groups**(txn_lag_tensor=800D dominant)이며 `docs/.../02_feature_engineering.md`, README 참조. 이 부록의 ~500D/12-group은 더 오래된 라우팅 관점으로 한정한다.
