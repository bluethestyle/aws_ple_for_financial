...
Stage 5    레이블 파생        LabelDeriver: 13개 태스크 레이블 생성
...
Tier 3 — Product Group (6)    nba_label 인덱스에서 그룹별 이진 분류
Tier 5 — Txn NBA (3 tasks)    txn_mcc_seq 거래 시퀀스에서 파생
```

> **2026-04-12**: has_nba (binary, Tier 1) → nba_primary (multiclass) 통합. nba_primary class 0 = no NBA. 총 13 tasks (binary 7개, multiclass 3개, regression 3개). 역사적 축소: 18→14→13.
...
lifecycle   "고객이 어디에 있는가"  [churn_signal, product_stability, segment_prediction]
            intra=0.7, inter=0.3
