...
AWS 합성 데이터 ablation(Santander 941K x ~349D input / 403D after Phase 0, benchmark v12)에서 프레임워크 작동을 검증했다.
온프렘 실험의 목적은 **실제 금융 운영 데이터(12M x 734D)에서 아키텍처 효과를 검증**하는 것이다.

### 합성 데이터에서 확인된 것 (arXiv v1)
- Expert별 차등 기여 존재 (TDA→investments, HGCN→deposits via MCC hierarchy, Causal→churn)
...
|------|-----------|-------------|
| 고객 수 | 941K | ~12M |
| 피처 차원 | ~349D input / 403D after Phase 0 (benchmark v12) | 734D (16 tasks) |
...
**근거**: 합성 데이터 benchmark v4에서 AUC 1위 (0.5771). adaTT 없이도 최고 성능. ※ benchmark v12에서 softmax > sigmoid로 역전되었으므로, 온프렘에서는 ple_softmax도 추가 검증 권장.
**검증**:
...
|------|-----------|---------|-----------|
| Binary | churn_signal, top_mcc_shift, will_acquire_* | **AUC** | F1 (optimal threshold) |
| Multiclass | nba_primary, next_mcc | **F1 macro** | Accuracy |
| Regression | product_stability, mcc_diversity_trend | **MAE** | RMSE |
...
### Joint Ablation (FP32, 10ep, Santander 941K x ~349D input / 403D after Phase 0, benchmark v4 historical)
...
### Structure Ablation (FP32, 10ep) — benchmark v4 historical

> **주의**: 아래는 benchmark v4 historical 결과. benchmark v12에서 softmax > sigmoid로 역전됨 (CGC gate 이종 MTL 환경 실험 결과).

| Scenario | AUC | 비고 |
|----------|-----|------|
| ple_sigmoid | 0.5771 | **v4 기준 1위 (adaTT 없이)** |
| adatt_only | 0.5765 | PLE 없이 adaTT만 |
| shared_bottom | 0.5726 | baseline |
| ple_softmax | 0.5684 | v4 기준 sigmoid 대비 -0.009 (v12에서 역전됨) |
