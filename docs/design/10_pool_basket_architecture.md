...
|---|-----------|--------|------|-----------|------|
| 1 | `mlp` | `MLPExpert` | `core/model/experts/mlp.py` | 전체 피처 | 기본 MLP 베이스라인 |
| 2 | `deepfm` | `DeepFMExpert` | `core/model/experts/deepfm.py` | 전체 피처 | FM + Deep 피처 상호작용 |
| 3 | `mamba` | `MambaExpert` | `core/model/experts/mamba.py` | 시퀀스 | Selective State Space Model |
| 4 | `temporal_ensemble` | `TemporalEnsembleExpert` | `core/model/experts/temporal.py` | 시계열 | Mamba + Transformer 앙상블 |
| 5 | `causal` | `CausalExpert` | `core/model/experts/causal.py` | 전체 피처 | NOTEARS DAG 인과 구조 학습 |
| 6 | `optimal_transport` | `OptimalTransportExpert` | `core/model/experts/ot.py` | 전체 피처 | Sinkhorn 최적 수송 |
| 7 | `hgcn` | `UnifiedHGCNExpert` | `core/model/experts/hgcn.py` | 그래프 | 쌍곡 그래프 합성곱 |
| 8 | `perslay` | `PersLayExpert` | `core/model/experts/perslay.py` | TDA 피처 | 위상 데이터 분석 |
| 9 | `lightgcn` | `LightGCNExpert` | `core/model/experts/lightgcn.py` | 그래프 | 경량 그래프 합성곱 (협업 필터링) |
