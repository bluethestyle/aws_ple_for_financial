...
|---|-----------|--------|------|-----------|------|
| 1 | `mlp` | `MLPExpert` | `core/model/experts/mlp.py` | 전체 피처 (403D raw columns after Phase 0, 라우팅 없음) | 기본 MLP 베이스라인 (태스크 전용) |
| 2 | `deepfm` | `DeepFMExpert` | `core/model/experts/deepfm.py` | State 피처 (109D) | FM + Deep 피처 상호작용 |
| 3 | `mamba` | `MambaExpert` | `core/model/experts/mamba.py` | 시퀀스 | Selective State Space Model |
| 4 | `temporal_ensemble` | `TemporalEnsembleExpert` | `core/model/experts/temporal.py` | Timeseries 피처 (129D) | Mamba + Transformer 앙상블 |
| 5 | `causal` | `CausalExpert` | `core/model/experts/causal.py` | Snapshot+State 피처 (103D) | NOTEARS DAG 인과 구조 학습 |
| 6 | `optimal_transport` | `OptimalTransportExpert` | `core/model/experts/ot.py` | Snapshot+State 피처 (69D) | Sinkhorn 최적 수송 |
| 7 | `hgcn` | `UnifiedHGCNExpert` | `core/model/experts/hgcn.py` | **merchant_hierarchy (47D = 20D hyperbolic + 27D merchant_dim)** — MCC 계층 구조 학습 | 쌍곡 그래프 합성곱 |
| 8 | `perslay` | `PersLayExpert` | `core/model/experts/perslay.py` | TDA 피처 (32D) | 위상 데이터 분석 |
| 9 | `lightgcn` | `LightGCNExpert` | `core/model/experts/lightgcn.py` | **product_hierarchy + graph_collaborative** — 상품 계층 + 협업 필터링 | 경량 그래프 합성곱 (협업 필터링) |
...
Expert Pool (11종)
    │
    ▼  ExpertBasket 선택
Expert Basket (7 shared + 1 task)
    │
    ▼  FeatureRouter (활성화 — build_model()에서 자동 생성)
        feature_groups.yaml target_experts 선언 → 전문가별 이종 입력 차원
        deepfm: 109D │ temporal_ensemble: 129D │ hgcn: 47D (20D hyperbolic + 27D merchant_dim) │ perslay: 32D
        causal: 103D │ lightgcn: product_hierarchy+graph_collaborative │ optimal_transport: 69D
        mlp (task): 51D
    │
    ▼  CGCLayer.forward()
...
> **파라미터 효과**: FeatureRouter 활성화로 모델 파라미터 4.77M → ~2.8M (감소). 각 전문가가 관련 피처 축만 수신하므로 노이즈 입력이 제거된다.

### 현재 금융 파이프라인 설정

`configs/pipeline.yaml`(canonical 경로)에 정의된 현재 설정. YAML 키는 `shared:`/`task:`이며, Python 데이터클래스 필드명(`shared_experts`/`task_experts`)과는 구분된다:

```yaml
expert_basket:
  shared:                  # 7종 선택 (Pool 11종 중)
    - deepfm
    - temporal_ensemble
    - hgcn
    - perslay
...
>   target_experts: [hgcn]                                # MCC 계층 (47D = 20D hyperbolic + 27D merchant_dim) → HGCN 전용
>
> - name: product_hierarchy
>   target_experts: [lightgcn]                            # 상품 계층 + 협업 필터링 → LightGCN
> ```

미선택 전문가 (`mamba`, `autoint`, `xdeepfm`) 는 Pool에 등록되어 있지만 이 파이프라인의 production basket에는 포함되지 않는다(registered but not in production basket). `num_shared_experts: 7`, `num_task_experts: 1`. 도메인 전환 시 Basket만 교체하면 된다.

---

## 3. Feature Generator Pool/Basket (구현 완료)

### 현재 상태

Feature Generator는 Pool/Basket 구조가 완전히 구현되어 있다:
- **Pool**: `FeatureGeneratorRegistry`에 Generator를 등록. 구체 Generator 클래스와 `@register` 데코레이터는 `core/feature/generators/*.py` 패키지에 위치한다(`core/feature/generator.py`는 베이스 클래스/레지스트리 정의).
- **Basket**: `feature_groups.yaml`이 어떤 Generator를 사용할지 선택 (Basket 역할)
- **Runtime**: `FeatureGroupPipeline`이 선택된 Generator를 실행

### Pool -- 등록된 Generator (16종 등록, 패키지 docstring 기준 14종 열거)

`core/feature/generators/` 패키지를 임포트하면 각 모듈의 `@FeatureGeneratorRegistry.register` 데코레이터가 실행되어 Generator가 등록된다. tda 모듈은 3종(`tda`, `tda_global`, `tda_local`)을 등록하므로 등록 클래스 총합은 16종이며, 패키지 `__init__` docstring은 14종을 열거한다.

| # | 등록 이름 | 파일 | 설명 |
|---|-----------|------|------|
| 1 | `tda` / `tda_global` / `tda_local` | `core/feature/generators/tda.py` | 위상 데이터 분석 (Persistence Diagram) — 3종 등록 |
| 2 | `phase_transition` | `core/feature/generators/phase_transition.py` | Persistence diagram 거리 기반 위상 전이 탐지 |
| 3 | `hmm` | `core/feature/generators/hmm.py` | 삼중 모드 HMM 상태 추정 (journey/lifecycle/behavior) |
| 4 | `gmm` | `core/feature/generators/gmm.py` | BIC 검증 기반 GMM 소프트 클러스터링 |
| 5 | `graph` | `core/feature/generators/graph.py` | LightGCN 그래프 임베딩 (옵션: Poincaré 투영) |
| 6 | `mamba` | `core/feature/generators/mamba.py` | Mamba SSM 시계열 임베딩 |
| 7 | `multidisciplinary` | `core/feature/generators/multidisciplinary.py` | 화학동역학, 전염병확산, 간섭, 범죄패턴 |
| 8 | `temporal` | `core/feature/generators/temporal.py` | 시계열 롤링 집계, 주기 인코딩, velocity |
| 9 | `economics` | `core/feature/generators/economics.py` | 경제/금융행동 피처 (소득 분해 + 금융행동) |
| 10 | `merchant_hierarchy` | `core/feature/generators/merchant_hierarchy.py` | MCC 계층, 브랜드 SVD, 집계 통계, radius |
| 11 | `model_features` | `core/feature/generators/model_features.py` | 모델 파생 피처 (HMM 요약 + Bandit/MAB + LNN, 27D) |
| 12 | `lag_extractor` | `core/feature/generators/lag_extractor.py` | LIST 시퀀스 K-step 우정렬 lag flattening (axis-1) |
| 13 | `rolling_stats_extractor` | `core/feature/generators/rolling_stats_extractor.py` | LIST 시퀀스 롤링 윈도우 통계 (axis-2) |
| 14 | `topn_multihot_extractor` | `core/feature/generators/topn_multihot_extractor.py` | LIST<int> Top-N/고정 vocab multi-hot (axis-3) |

> `economics`와 `model_features`는 이미 구현·등록되어 있다(과거 "추가 필요" 목록에서 제외).

### Basket -- feature_groups.yaml

Basket은 Pool에서 파이프라인이 사용할 Generator를 `generator` 필드로 참조한다. canonical 경로는 `configs/santander/feature_groups.yaml`(17개 feature group, 1211D — txn_lag_tensor 800D dominant)이다. 각 group이 `generator` 필드로 Pool의 Generator를 참조한다:

```yaml
# generate 타입 -- Generator가 피처를 생성
- name: tda_topology
  group_type: generate
  generator: tda                  # Pool에서 선택
  generator_params: { ... }
...
현재 16종이 모두 등록·구현되어 있으므로(`economics`, `model_features` 포함), 새 도메인이 추가 Generator를 요구할 때만 같은 패턴으로 확장한다:
...
```python
# 새 Generator 추가 예시 (core/feature/generators/<name>.py)
@FeatureGeneratorRegistry.register("my_extractor")
...
| `pipeline.yaml` tasks | `configs/pipeline.yaml` | Basket 역할 (12개 태스크 선택) |
...
핵심: **코드 한 줄 변경 없이**, config 디렉터리만 교체하면 전혀 다른 도메인의 추천 시스템이 구성된다. 현재 canonical 도메인 config는 `configs/santander/`(feature_groups 17 groups/1211D, pipeline 12 tasks)이며, 위 `configs/ecommerce/`는 도메인 전환을 보이기 위한 illustrative/legacy 예시다. (참고: `configs/financial/`는 santander 이전의 도메인-스왑 예시 디렉터리로, ingestion/agent 등 일부 도메인 자료만 보존된 legacy 경로이며 학습 파이프라인 canonical은 `configs/santander/`이다.)
...
| **Phase 2** | Task Group 통합 (`TaskGroupConfig` + adaTT 연동) | 다음 | `core/model/ple/config.py`, `configs/pipeline.yaml` |
| **Phase 3** | Feature Generator 확장 (새 도메인 요구 시 신규 Generator 추가) | 도메인 확장 시 | `core/feature/generators/` |
| **Phase 4** | Task active/disabled 토글, Feature Pool 정규화 | 파이프라인 실행 후 | `core/pipeline/config.py`, `core/feature/generators/` |

```
Phase 1 (완료)          Phase 2 (다음)           Phase 3              Phase 4
─────────────────      ─────────────────       ─────────────────    ─────────────────
Expert Pool/Basket     TaskGroupConfig         Feature Generator    Task 토글
ExpertBasketConfig     pipeline.yaml에          신규 도메인          active/disabled
ExpertBasket           task_groups 정의          Generator 확장       Feature Pool
CGCLayer 연동          adaTT/loss 공통 참조      (필요 시)            정규화
```
...
| Feature | `FeatureGeneratorRegistry` (16종 등록) | `feature_groups.yaml` | 아직 불필요 |
