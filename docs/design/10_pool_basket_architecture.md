...
|---|-----------|--------|------|-----------|------|
| 1 | `mlp` | `MLPExpert` | `core/model/experts/mlp.py` | 전체 피처 (~349D input, 403D after Phase 0, 라우팅 없음) | 기본 MLP 베이스라인 (태스크 전용) |
...
| 7 | `hgcn` | `UnifiedHGCNExpert` | `core/model/experts/hgcn.py` | **merchant_hierarchy (27D Poincaré)** — MCC 계층 구조 학습 | 쌍곡 그래프 합성곱 |
...
Expert Basket (7 shared + 2 task)
    ...
    ▼  FeatureRouter (활성화 — build_model()에서 자동 생성)
        ...
        deepfm: 109D │ temporal_ensemble: 129D │ hgcn: 27D (merchant_hierarchy Poincaré) │ perslay: 32D
...
>   target_experts: [hgcn]                                # MCC Poincaré 27D → HGCN 전용
>
...
| `pipeline.yaml` tasks | `configs/financial/pipeline.yaml` | Basket 역할 (13개 태스크 선택) |
