...
Stage 5:   LabelDeriver (12 tasks, derived in generate_benchmark_data.py to prevent spend_level-style leakage)
...
`core/pipeline/label_deriver.py`가 config-driven으로 12개 레이블을 생성한다.
...
| `categorical_encode` | 범주형 → 정수 인코딩 | `next_mcc` (string MCC → int) |
...
| 레이블 생성 | 코드 내 하드코딩 | **generate_benchmark_data.py에서 파생 (v4 기준), LabelDeriver (config-driven, 12 tasks)** — 정규화 이전에 파생하여 spend_level 스타일 결정론적 누수 방지 | 선언적, 재현 가능 |
