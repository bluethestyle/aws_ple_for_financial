...
### 팩트 추출 레이어 (2026-04 추가)

`FactExtractor`가 `InterpretationRegistry`를 보완한다. 전자는 고객 레벨 *서술적 프로파일*("예적금 중심 포트폴리오", "리스크 회피 성향")을, 후자는 피처 레벨 한국어 해석을 제공한다.

**통합 흐름**:
1. **Phase 0 배치**: `FactExtractor.extract_batch()`가 모든 고객에 대해 룰 기반 팩트 추출 (LLM 없음)
2. **저장**: `ContextVectorStore.build()`의 metadata 파라미터로 `{cid: {"customer_facts": [...]}}` 전달
...
**안전성**: Python `eval()`이 `__builtins__` 차단 샌드박스에서 실행되므로 config 파일 변조에도 임의 코드 실행 불가. `validate_rules()` 메서드로 pre-flight 검증 가능.

상세 설계: `docs/design/11_ops_audit_agent.md` §메모리 프레임워크 선택적 차용
