...
**기준 시점**: 2026-04-28
**AWS 기준**: `aws_ple_for_financial` main 브랜치 (commit `88f7a7b` 까지, 로컬 main 미푸시 포함)
...
- 누적 테스트: **831/831 PASS** (2026-06-26 실측, OCP 커밋 50b02b6 후) — 에이전트 sync 트랙 Phase 1~4 신규 테스트 + OCP/CCA 트랙 신규 테스트 반영. 종전 639/771/785 표기는 각각 2026-06-10/06-12/06-12 중간 시점의 dated superseded 스냅샷. 하드코딩: agent_dialog 경로(`_attach_llm_followup`)와 Titan embedding model id 는 2026-06-12 config 주입 완료, `_BedrockProvider`(consensus provider)의 `ap-northeast-2`/model_id 도 같은 날 config 주입 완료(P3 해소 — `llm_provider.bedrock.models.agent_consensus.model_id` + region 은 `llm_provider.bedrock.region` → `aws.region` → env 순, 실패 시 종전 기본값 폴백).
...
- `docs/onprem_to_aws_import_plan.md` — 에이전트 sync 트랙 (온프렘 → AWS 이식 15건 상태 보드)
...
## 8. Phase 0 Schema Audit + Mamba Precompute (2026-04-26~28)

본 섹션은 본 문서가 다루는 compliance sync 범위와는 별개의 **데이터/피처 파이프라인 인프라 보강**으로, 같은 시기에 main 브랜치에 누적된 변경사항을 추적용으로 정리합니다 (로컬 commit, 아직 origin push 안 됨).

### 8.1 Phase 0 Schema Audit — invariant 위반 6종 차단

Phase 0 출력 (`features.parquet`, `feature_schema.json`) audit 결과 FeatureRouter / config_builder 가 silently 잘못된 슬라이스를 잡아낼 수 있는 invariant 위반 6종 발견 + 차단:

1. `core/feature/group_pipeline.py` — fit 후 `output_columns` 강제 overwrite (placeholder 잔존 차단).
2. `core/preprocessing/normalizer.py::transform_sql` — 입력 컬럼 순서 보존 (Stage 6 binary reorder 가 group range 비연속화시키던 버그 차단).
3. Stage 6 종료 시 `feature_cols` 를 feature group registry 순서로 재정렬.
4. `config_builder` + FeatureRouter 양쪽에 defensive OOB guard 추가.
5. `feature_groups.yaml::output_dim` 6개 그룹 정렬 — demographics 38→11, tda_global 36→16, tda_local 24→16, hmm_states 48→25, product_hierarchy 32→34, graph_collaborative 64→66.
6. `FeatureSpec.meta_cols` / `TaskSpec.derive` 가 dataclass 에 없어 dead config 였음 → 필드 추가 + Phase 0 에서 meta_cols 분리 보장 (`snapshot_date`, `has_nba` 가 `features.parquet` 으로 더 이상 leak 되지 않음).

**검증**: Phase 0 v3 (commit `88f7a7b`) — 1211 features / 17 groups 모두 sequential / 마지막 range 1205 종료 / trailing 6 passthrough cols.

### 8.2 Mamba precompute — 별도 SageMaker Job 으로 분리
...
- **이미지 빌드**: `bash scripts/build_mamba_image_codebuild.sh` (AWS CodeBuild + cu122-torch2.1 사전 빌드 wheel: causal_conv1d 1.2.0.post2, mamba_ssm 1.2.0.post1).
...
- **실측**: g4dn.xlarge spot, **8.6 분 / $0.026/run** (Phase 0 본 잡과 별도 과금).

### 8.3 비용 실측

| 잡 | 실측 |
|---|---|
| Phase 0 single-job (CPU instance, spot) | ~7 분 / $0.04/run |
| Mamba precompute (g4dn.xlarge spot) | 8.6 분 / $0.026/run |

**4월 누적**: ~$22.55 (CodeBuild $2.49 / 16 attempts, SageMaker $12+ Phase 0 + Mamba precompute, S3 $1.22 등). CodeBuild 빌드 시도 비용은 mamba wheel 사전 빌드 전략으로 전환 후 1회 빌드 / 다회 재사용 구조로 안정화.
