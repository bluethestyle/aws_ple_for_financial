...
#block(
  ...
  추가로 adaTT(Adaptive Task-aware Transfer)를 통한 16개 태스크 간 동적 지식 전이 메커니즘과
...
Section 3 (Hyperbolic GCN Expert)에서 LightGCN의 message passing, layer combination, BPR loss를 상세히 기술하였다.

핵심 수학은 Hyperbolic GCN Expert 절에서 동일하게 다루었으므로 여기서는 금융 도메인 적용에 집중한다.
...
(16개 태스크 $times$ $N$개 전략 = 실행 불가), 느리고 (수 주 소요),
집단 수준 ATE만 제공한다.
...
16개 동시 태스크(CTR, CVR, Churn, NBA 등)가 전문가 파라미터를 공유할 때
gradient 충돌이 negative transfer를 야기한다.
고정 타워 MTL의 세 가지 근본적 한계:
(1) 공유 backbone이 모든 태스크에 동일하게 영향 --- CTR 최적화가 Churn 예측을 악화시켜도 감지/방지 메커니즘 없음,
(2) 16개 태스크 간 어떤 쌍이 서로 돕는지/해치는지 측정 불가,
...
#figure(
  table(
    ...
    [engagement], [ctr, cvr, engagement, uplift], [0.8], [고객 관여/전환],
    [lifecycle], [churn, retention, life\_stage, ltv], [0.7], [고객 생애주기],
    [value], [balance\_util, channel, timing], [0.6], [고객 가치/행동],
    [consumption], [nba, spending\_category, ...], [0.7], [소비 패턴],
  ),
