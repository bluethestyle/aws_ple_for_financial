  ...
  #block(width: 85%)[
    ...
    #grid(
      ...
      align(left)[
        ...
        #text(9pt, fill: slate)[PLE-Cluster-adaTT Architecture]
      ],
      align(right)[
...
AIOps 추천 시스템은 CTR, CVR, Churn, LTV 등 16개 태스크를 동시 예측해야 한다.
...
CGC는 태스크별 독립 gate가 Shared Expert 출력에 서로 다른 가중치를 적용한다.
본 구현에서는 8개 이종 Shared Expert에 대해 *블록 스케일링* 방식을 사용한다.
...
CGC gate의 Softmax 가중합은 Transformer Attention과 본질적으로 같다:
*Query*(gate weight), *Key*(shared representation), *Value*(Expert output)의 관계로
"관련성에 비례하여 정보를 선택적으로 결합"한다.
...
#styled-table(
  ...
  [Behavior], [16D], [monthly], [NBA, Balance\_util, Channel, Timing, ...],
)
...
16개 태스크가 Shared Expert 파라미터를 공유할 때, gradient 충돌로 인한
...
진단 API: `detect_negative_transfer()`가 `{"churn": ["ctr", "engagement"]}` 형태로
...
도메인 지식에 기반하여 16개 태스크를 4개 그룹으로 분류한다:

#styled-table(
  (1fr, 2.5fr, 0.7fr, 0.7fr, 1.8fr),
  [*그룹*], [*멤버*], [*Intra*], [*Inter*], [*비즈니스 의미*],
  [Engagement], [ctr, cvr, engagement, uplift], [0.8], [0.3], [고객 참여/전환],
  [Lifecycle], [churn, retention, life\_stage, ltv], [0.7], [0.3], [고객 생애주기],
  [Value], [balance\_util, channel, timing], [0.6], [0.3], [고객 가치/행동 패턴],
  [Consumption], [nba, spending\_category, consumption\_cycle,\ spending\_bucket, merchant\_affinity, brand\_prediction], [0.7], [0.3], [소비 패턴 분석],
)

*Intra-group strength*: 같은 그룹 내 태스크 간의 전이 강도. Engagement 그룹(0.8)이 가장 높다 --- CTR과 CVR은 퍼널 관계로 강한 positive transfer 기대.
...
#styled-table(
  ...
  [CTR], [CVR], [Sequential], [0.5], [클릭 $arrow.r$ 전환 퍼널],
  [Churn], [Retention], [Inverse], [0.5], [이탈 확률의 역 = 유지],
  [CVR], [LTV], [Feature], [0.5], [전환 확률 $arrow.r$ 생애가치],
  [NBA], [Spending\_category], [Feature], [0.5], [추천 행동 $arrow.r$ 소비 카테고리],
  [Spending\_category], [Brand\_prediction], [Feature], [0.5], [소비 카테고리 $arrow.r$ 브랜드 선택],
)
...
16개 태스크의 가중치를 수동 튜닝하는 조합 폭발을 *자동 균형*으로 대체한다.
...
#styled-table(
  ...
  [ctr], [1.0], [$gamma$=2, $alpha$=0.25], [표준],
  [cvr], [1.5], [$gamma$=2, $alpha$=0.20], [양성 비율 극소 $arrow.r$ weight 상향],
  [churn], [1.2], [$gamma$=2, $alpha$=0.60], [FN 비용 높음 $arrow.r$ alpha 상향],
  [nba], [2.0], [CE (12 classes)], [비즈니스 핵심],
  [ltv], [1.5], [huber ($delta$=1.0)], [regression, outlier robust],
  [brand\_pred], [2.0], [contrastive], [InfoNCE, 50K brands],
)
...
#styled-table(
  ...
  [16 task gradients], [$tilde$0.3x], [각 gradient는 shared\_param\_size],
