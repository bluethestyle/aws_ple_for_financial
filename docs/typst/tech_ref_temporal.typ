...
// ─────────────────── 커스텀 컴포넌트 ────────────────
#let note-box(title, body, accent: teal) = {
  block(
    width: 100%,
...
#let warn-box(title, body) = note-box(title, body, accent: amber)
...
#let dim(body) = text(size: 8.5pt, fill: slate)[#body]
...
#align(center)[
  ...
  #text(size: 9pt, fill: slate)[v1.0 · 2026-04-01]

  #v(3em)

  #block(width: 85%, inset: 1em, stroke: (left: 2pt + slate.lighten(40%)))[
    #text(size: 9.5pt)[
