...
#align(center)[
  ...
  #text(size: 14pt, fill: anthropic-muted)[
    Technical Reference: NOTEARS 인과 추론과 Sinkhorn 최적 수송
  ]
...
#block(
  ...
  Optimal Transport (OT) Expert의 Theoretical Background, 수학적 구조, 구현 상세를 기술한다.
  Causal Expert는 Pearl의 구조적 인과 모형(SCM)과 NOTEARS 연속 비순환성 제약을
  결합하여 피처 간 인과 관계를 학습하며, OT Expert는 Sinkhorn Entropy Regularization
...
Judea Pearl은 인과추론의 인식론적 수준을 세 Stage로 Category하였다.

*제1Stage: 연관(Association).* 관측 데이터에서 변수 간 상관관계를 파악한다.
$P(Y | X)$ --- "X를 관측했을 때 Y의 확률은 어떻게 변하는가." 전통적 추천 시스템은
이 수준에서 동작한다. "프리미엄 카드 보유자가 여행 보험 가입률이 높다"는 연관이며,
인과 방향을 Category하지 않는다.

*제2Stage: 개입(Intervention).* 변수를 인위적으로 조작했을 때의 효과를 추정한다.
...
*제3Stage: 반사실(Counterfactual).* "만약 다른 Action를 취했다면 결과가 달랐을까?"
...
Causal Expert는 인접행렬 $bold(W)$를 통해 변수 간 인과 방향과 강도를 학습하여, 개입 수준(제2Stage)의 추론 구조를 추천에 내재시킨다. $W_(i,j)^2$는 "변수 $j$를 개입하여 변경하면 변수 $i$가 얼마나 변하는가"를 인코딩한다.
...
기존의 제약 기반(PC 알고리즘) 또는 스코어 기반(GES) Approach은 조합 탐색의 Limitation를 벗어나지 못했다.
...
Compressor는 이를 32개 핵심 인과 변수로 Summary하여 DAG 학습의 계산 부담을 해소한다.
...
각 고객의 잠재 Characteristics 벡터 $bold(z)$를 "다른 Characteristics들이 미치는 인과적 영향"만큼 보정한 버전 $hat(bold(z))$를 생성한다. 이를 통해 단순 상관이 아닌 인과적으로 조정된 표현을 얻는다.

인접행렬 $bold(W)$는 `torch.randn(32, 32) * 0.01`로 소규모 초기화되어 학습 초기 안정성을 보장한다.

=== Causal Encoder

인과 개입된 표현을 최종 Output Dimension으로 변환한다.
...
#block(
  ...
  #text(weight: "bold")[Caution:] `dag_lambda` $> 0.1$로 설정하면 인접행렬 $bold(W)$가 영행렬로 수렴하여 인과 구조 자체가 소멸한다. DAG Regularization Loss이 태스크 손실보다 지배적이 되면 Expert가 항등 함수 ($hat(bold(z)) approx bold(z)$)로 퇴화한다.
]
...
- *Description력 강화*: `get_causal_graph()`로 인접행렬 $bold(W) circle.tiny bold(W)$를 추출하여, 어떤 잠재 인과 변수가 다른 변수에 강하게 영향을 미치는지 히트맵으로 시각화할 수 있다.
...
#table(
  ...
  inset: 8pt,
  stroke: 0.5pt + luma(200),
  table.header[*Item*][*논문 (Zheng et al.)*][*본 구현*],
  [Purpose], [관측 데이터에서 DAG 구조 학습], [Expert 내 피처 인과 관계 학습],
  [변수 수], [수십~수백 (범용)], [32 고정],
...
이들은 공통적으로 분포의 기하학적 구조를 무시한다는 Limitation가 있다. 분포 $P$가 "서울"에 집중, $Q$가 "인천"에 집중, $R$이 "부산"에 집중되어 있을 때, KL 발산이나 TV 거리로는 $"dist"(P, Q) approx "dist"(P, R)$이다 --- 질량이 겹치지 않으면 거리가 동일하다. 그러나 직관적으로 서울-인천은 서울-부산보다 가깝다.
...
#table(
  ...
  inset: 8pt,
  stroke: 0.5pt + luma(200),
  table.header[*Property*][*KL 발산*][*Wasserstein 거리*],
...
=== Kantorovich 문제의 계산적 Limitation
...
*구현 Note:* 이론적 정의에서 $bold(v)$ 업데이트는 $bold(C)^top$을 사용하지만, 본 구현에서는 PSD Cost Matrix($bold(C) = bold(M)^top bold(M)$)이 대칭이므로 $bold(C)^top = bold(C)$이다. 코드에서는 전치 대신 `logsumexp`의 축(dim)을 변경하여 동일한 결과를 얻는다.
- 반복 횟수: 10회 (Default, `sinkhorn_iterations`)
...
#table(
  ...
  inset: 8pt,
  stroke: 0.5pt + luma(200),
  table.header[*Item*][*논문 (Cuturi 2013)*][*본 구현*],
...
g4dn T4 GPU에서 AMP (Automatic Mixed Precision)를 활성화하면 학습 속도가 약 2배 향상되지만, FP16의 표현 범위 제한 ($approx 6 times 10^(-8)$ ~ $6.5 times 10^4$)으로 인해 Numerical Stability Issues가 발생한다.

== Causal Expert FP16 Issues

=== Overflow in Taylor Approximation

NOTEARS Taylor 10-Term Approximation에서 $bold(M)^k$ 누적 행렬곱은 $k$가 증가할수록 원소 크기가 급격히 증가할 수 있다. FP16에서 $6.5 times 10^4$를 초과하면 `inf`가 되고, `trace`에서 NaN으로 전파된다.
...
#table(
  ...
  inset: 8pt,
  stroke: 0.5pt + luma(200),
  table.header[*Expert*][*Symptom*][*Cause 및 해결*],
  [Causal],
...
#block(
  ...
  #text(weight: "bold")[FP16 원칙:] 두 Expert 모두 핵심 수치 연산 (Taylor 행렬 지수, Sinkhorn log-domain)은 FP32로 수행하고, 입출력 텐서만 FP16으로 캐스팅한다. AMP의 `GradScaler`가 NaN gradient를 감지하면 해당 step을 자동 스킵하지만, 빈번한 스킵은 학습 품질을 저하시키므로 근본 Cause을 해결해야 한다.
]
...
#table(
  ...
  [Input Dimension], [644D (정규화 피처)], [644D (정규화 피처)],
  [Latent Space], [32D (인과 변수)], [32D (확률 simplex)],
  ...
  [Learnable Parameters], [$bold(W)$ `[32,32]` 인접행렬], [`reference_logits [16,32]` + `cost_matrix [32,32]`],
  [Regularization Loss], [$cal(L)_("DAG")$ (비순환성 + 희소성)], [없음 (Entropy Regularization 내재)],
  [Interpretable Output], [4D (인과 강도 등)], [4D (수송 비용 등)],
)
...
Causal Expert만 추가 Regularization Loss을 가진다:
...
OT Expert는 별도 Regularization Loss 없이 태스크 손실의 역전파를 통해 `reference_logits`와 `cost_matrix`가 자연스럽게 학습된다.


== 세 Expert의 수학적 Perspective 비교
...
$bold(W)$는 태스크 손실과 DAG Regularization Loss 양쪽에서 gradient를 받는다. 두 gradient의 균형이 `dag_lambda` 설정으로 제어된다.
