...
#table(
  ...
  [lifecycle], [Where is the customer?], [churn\_signal, product\_stability], [0.7], [0.3],
...
=== 13 Generated Groups (generate type)
...
== Phase 1--3: Ablation Study (23 Scenarios, 48 Total Configurations)
...
#figure(
  placement: auto,
  {
    ...
    fletcher.diagram(
      ...
      edge(<logit>, <towers>, "->"),
      node((0, 7), [*Task Towers × 12 (TowerRegistry)* \ #text(size: 6pt)[Evidential Layer (regression) · SAE sidecar] \ #text(size: 6pt)[Per-task Loss + Uncertainty Weighting]], width: 68mm, fill: out-fill, name: <towers>),
    )
  },
...
- `tasks`: 12 tasks (name, type, loss, loss\_weight, label\_col)
