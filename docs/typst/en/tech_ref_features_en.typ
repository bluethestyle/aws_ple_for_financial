// =============================================================================
//  피처 엔지니어링 Technical Reference — AWS PLE for Financial
//  v1.0 · 2026-04-01
// =============================================================================
...
#set page(
  ...
  header: context {
    if counter(page).get().first() > 1 [
      ...
      #smallcaps[피처 엔지니어링 Technical Reference]
      #h(1fr)
...
#align(center)[
  ...
  #block(width: 85%, stroke: (left: 2pt + anthropic-accent), inset: (left: 14pt, right: 14pt, top: 8pt, bottom: 8pt))[
    #text(size: 9.5pt, fill: anthropic-muted)[
      ...
      그리고 3-Stage Normalization Pipeline을 포함한다.
    ]
  ]
]
...
#v(12pt)
#warn[설계 vs 구현 차원 안내][
  본 문서는 *풀뱅크 설계(734D)*를 기준으로 작성되었습니다. 현재 Santander 벤치마크 구현은 *316D (12 feature groups)*입니다. 실제 구현의 차원 명세는 `outputs/phase0/feature_schema.json`을 참조하십시오. Appendix "설계 vs 구현 차원 매핑"에서 각 그룹별 차이를 확인할 수 있습니다.
]
...
#styled-table(
  ...
  [파동 물리학], [주기 분해, 위상 Motivation화], [FFT 주파수 영역, PLV 위상 결합],
  [TDA], [위상적 형태, 구멍, 연결성], [좌표 불변 구조, 다중 해상도 관찰],
  [HMM], [숨겨진 상태 전이], [관측 불가능한 잠재 Stage의 확률적 추론],
...
Multidisciplinary Features 엔지니어링에서 "아날로지"는 단순한 비유가 아니다. *구조적 동형사상(structural isomorphism)*에 기반한다. 두 시스템의 표면적 대상(분자 vs 소비자)은 다르지만, 그 대상들 사이의 _관계 구조_가 수학적으로 동일할 때 구조적 동형사상이 성립한다.
...
본 시스템의 피처는 두 가지 Role을 동시에 수행한다.
...
#styled-table(
  ...
  [`savings_propensity`], [$(Y - C) / Y$], [저축 성향; 미래 Limitation효용 기대 반영],
...
Taylor 전개에서 $f'$ 항이 상쇄되어 유도되며, 가속(양)과 감속(음)을 Category한다.
...
주파수 분해능 상관으로, 어떤 주기에서 카테고리가 Motivation화되는지 식별한다.
...
#styled-table(
  ...
  [`phase_locking_value`], [PLV. 카테고리 간 소비 리듬의 위상 Motivation화],
  [`cross_coherence`], [교차 스펙트럼 코히어런스. 주파수별 카테고리 상관],
  [`hhi_shift`], [HHI 변화량. 소비 집중화/분산화 추세],
  [`category_sync_ratio`], [Motivation화 비율. 같은 위상의 카테고리 쌍 비율],
...
#note[TDA의 핵심 원리][
  데이터 포인트에 점점 커지는 공을 씌우면서, 연결 성분/구멍/빈 공간 같은 위상적 Characteristics이 _언제 나타나고 언제 사라지는지_를 추적한다. 오래 지속되는 Characteristics은 신호이고, 금방 사라지는 Characteristics은 노이즈이다.
]

TDA의 강점:
+ *좌표 불변성*: 데이터를 회전하거나 연속 변형해도 위상적 Characteristics 보존. 피처 스케일링에 강건.
+ *다중 해상도 관찰*: 단일 임계값이 아닌 모든 스케일을 동시에 고려.
+ *노이즈 내성*: 오래 지속되는 Characteristics만 취하여 허위 구조를 자연스럽게 필터링.
...
#styled-table(
  (1fr, 0.6fr, 2fr),
  table.header([*서브그룹*], [*차원*], [*Description*]),
...
#note[PersLay Expert와의 관계][
  본 문서의 TDA 피처(오프라인 70D)와 PersLay Expert(온라인 64D)는 모두 Persistent Homology를 활용하지만 Role이 다르다. 70D는 734D 메인 텐서의 일부이며 배치 전처리에서 사전 계산된다. PersLay Expert는 PLE 내부에서 Persistence Diagram을 end-to-end로 학습한다.
]
...
#note[HMM vs GMM][
  HMM은 _시계열_ 상태 추론이다. "이 고객이 현재 어떤 _Stage_에 있고, 다음에 어디로 이동하는가?" GMM은 _횡단면_ 클러스터링이다. "이 고객이 어떤 _유형_인가?" 전이 행렬 $bold(A)$가 시간 역학을 포착하므로, 같은 소비 패턴이라도 "성장 중"인지 "이탈 직전"인지를 Category한다. 두 모듈은 서로 다른 입력 경로로 PLE에 공급되어 상호 보완한다.
]
...
#eq-highlight[
  ...
  Architecture: $K = 20$ 클러스터, $D = 40$ Input Dimension, `covariance_type = "full"`.
]
...
#styled-table(
  (1.5fr, 0.6fr, 2.5fr),
  table.header([*피처*], [*차원*], [*Description*]),
...
#styled-table(
  ...
  [불확실성], [없음], [엔트로피 기반 신뢰도],
  [PLE Role], [단일 서브헤드 활성], [$gamma_(n k)$ 가중 앙상블로 20개 서브헤드 결합],
)
...
Mamba는 Continuous State Space Model(SSM)의 이산화에 기반한다:
...
#eq-highlight[
  ...
  Key point: $Delta_k$가 입력에 따라 *동적으로* 변하므로, 비정상 시계열에서도 적응적으로 상태를 업데이트한다.
]
...
#warn[온라인 Mamba Expert와의 Category][
...
== 시계열 분석의 4가지 Perspective

Mamba 50D는 아래 네 Perspective의 정보를 비선형으로 통합한 _학습된 표현_이다:

#styled-table(
  (1fr, 1.5fr, 1.5fr),
  table.header([*Perspective*], [*핵심 질문*], [*기법*]),
...
#styled-table(
  ...
  [학습 Purpose], ["누가 무엇을 좋아하는가"], ["가맹점이 구조적으로 어떻게 관련되는가"],
...
Normalization Pipeline은 세 Stage로 구성되며, *멱법칙(power-law) 분포를 가진 피처의 원본 크기 정보를 보존*하면서도 모델 학습에 적합한 스케일로 변환하는 것이 핵심 목표이다.

== Stage 1: Power-Law Detection + log1p 복사본 생성

#eq-highlight[
  *Power-Law Detection 기준*:
...
== Normalization Pipeline 흐름 Summary

```
원본 피처 → [Stage 1] Power-Law Detection (skew+kurt → log-log R²)
...
// =====================================================================
//  Appendix: 설계 vs 구현 차원 매핑
// =====================================================================
= Appendix: 설계 vs 구현 차원 매핑

#warn[참고][이 Appendix은 풀뱅크 설계(734D)와 현재 Santander 벤치마크 구현(316D) 간의 차원 차이를 정리한 것입니다. 구현 차원은 `outputs/phase0/feature_schema.json`에서 확인할 수 있습니다.]

#styled-table(
  (1.2fr, 1fr, 1fr, 2fr),
  table.header([*Feature Groups*], [*설계 (734D)*], [*구현 (316D)*], [*Notes*]),
