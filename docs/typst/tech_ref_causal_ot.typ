...
행렬 지수함수를 직접 계산하면 고유값 분해 등 $O(d^3)$ 연산이 필요하다. 구현에서는 Taylor 급수 10항으로 근사한다.

$ e^(bold(M)) approx sum_(k=0)^(9) frac(bold(M)^k, k!) = bold(I) + bold(M) + frac(bold(M)^2, 2!) + dots.c + frac(bold(M)^9, 9!) $ <taylor>

실제 구현에서는 누적 행렬곱을 사용한다:
...
$ bold(u)_("new") = log bold(mu) - "logsumexp"(-bold(C)/epsilon + bold(v)) $ <sinkhorn-u>
$ bold(v)_("new") = log bold(nu) - "logsumexp"(-bold(C)^top / epsilon + bold(u)) $ <sinkhorn-v>
...
*해결:*
- `clamp(min=1e-7)` 대신 `clamp(min=1e-4)`로 FP16 안전 범위 내 유지
