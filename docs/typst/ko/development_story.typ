...
#align(center)[
  #block(
    ...
    #text(size: 9pt, fill: anthropic-muted)[
      ...
      14-task, 7-expert PLE+adaTT 추천 시스템을 구축한 과정을 기록한다.
    ]
  ]
]
...
기존 금융 상품 추천 시스템은 ALS (Alternating Least Squares) 기반의 협업 필터링이었다. 이를 PLE (Progressive Layered Extraction) + adaTT (Adaptive Task Transfer) 아키텍처 기반의 멀티태스크 딥러닝 추천 시스템으로 교체하는 것이 목표였다. 14개 태스크를 7개 전문가 네트워크가 처리하며, 태스크 간 관계를 명시적으로 모델링하는 구조다.
...
4. *Loss 구조*: 온프렘은 uncertainty weighting을 먼저 적용 (loss scale 정규화) 후 adaTT transfer. AWS는 either/or로 구현되어 adaTT 활성화 시 uncertainty weighting이 꺼졌다. 14개 태스크의 loss scale이 제각각인 상태에서 transfer → 큰 loss가 지배.
...
태스크 수는 18개에서 14개로 줄었다. 남은 14개 태스크는 진정으로 불확실한 예측을
...
결과적으로 14개 태스크 전체에서 의미 있는 분포를 얻었다 — 학습할 무언가가
