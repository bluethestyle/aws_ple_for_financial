...
== adaTT가 빅테크에서는 작동하지만 금융에서는 아닌 이유

adaTT 결과는 더 깊은 질문을 불러일으켰다: KDD 2023에서 Meta Research가 발표한 방법이 왜 우리 환경에서는 실패하는가?

답은 _모집단 규모_ 데이터와 _표본 규모_ 데이터의 차이에 있다.
Meta는 일일 수십억 건의 상호작용 데이터를 운영한다. 이 규모에서 학습 데이터셋은 사실상 모집단이다 — 모델이 존재하는 거의 모든 패턴을 보기 때문에 과적합이 거의 불가능하다.
adaTT가 CTR과 CVR 태스크를 손실 수준 전이로 결합할 때, 이 결합은 수십억 샘플에서 안정적인 실제 구조적 관계(클릭이 구매에 선행)를 반영한다.
태스크 간 "상호 과적합"은 실제로는 정밀한 패턴 학습이며, 어떤 성능 향상이든 — 0.001 AUC라도 — Meta 규모에서는 직접적인 매출 증가로 전환된다.

금융 기관은 근본적으로 다른 체제에서 운영된다.
대형 리테일 은행이 1,000만~2,000만 고객을 보유하더라도, 생성되는 학습 데이터셋은 진화하는 고객 행동의 _표본_이지 모집단이 아니다.
금리 변동, 상품 정책 변경, 규제 변화는 과거 데이터에서 학습된 태스크 간 관계를 무효화하는 분포 이동을 유발한다.
adaTT가 이탈 예측과 상품 가입 예측을 결합할 때, 이 태스크들 간의 그래디언트 관계가 안정적이라고 가정한다.
하지만 금융에서는 금리 인상이 하룻밤 사이에 이 관계를 역전시킬 수 있다 — 상품을 가입하던 고객이 이탈하기 시작하고, 이전에 이탈하던 고객이 새로운 예금 금리에 끌릴 수 있다.

이는 구체적인 운영 리스크로 이어진다: 손실 수준 전이를 통한 태스크 결합은 _상관된 모델 실패(correlated model failure)_를 생성한다.
한 태스크의 분포가 이동하면, 결합된 손실이 연결된 모든 태스크로 오류를 전파한다.
분리된 아키텍처(독립 태스크 손실을 가진 PLE softmax)에서는 각 태스크가 독립적으로 이동하고 독립적으로 재학습하거나 모니터링할 수 있다.
모델의 실패 모드가 시스템적이 아닌 격리된 상태를 유지한다.

#quote-box()[
  adaTT 스타일의 결합과 PLE 스타일의 격리 사이의 선택은 궁극적으로 모델 수명 주기 관리에 대한 선택이다.
  빅테크는 시간 단위 재학습과 모집단 규모 데이터를 보유하기 때문에 긴밀한 결합을 감당할 수 있다.
  주간~월간 재학습과 표본 규모 데이터를 가진 금융 기관은 모델 수명을 연장하고 이동 전파를 억제하기 위해 아키텍처적 격리가 필요하다.
]

이 통찰 — _최적의 MTL 아키텍처는 태스크 구조뿐 아니라 운영 체제에 의존한다_ — 은 아마도 전체 어블레이션 캠페인에서 가장 실무적으로 유용한 발견일 것이다.
학술 벤치마크와 빅테크 배포에서 지배적인 방법이 규제되고 데이터가 적은 환경에서는 오히려 역효과를 낼 수 있는 이유를 설명해준다.
