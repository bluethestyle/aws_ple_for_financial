...
#align(center)[
  #block(
    ...
    #text(size: 9pt, fill: anthropic-muted)[
      ...
      12-task, 7-expert PLE+adaTT 추천 시스템을 구축한 과정을 기록한다.
    ]
  ]
]
...
기존 금융 상품 추천 시스템은 ALS (Alternating Least Squares) 기반의 협업 필터링이었다. 이를 PLE (Progressive Layered Extraction) + adaTT (Adaptive Task Transfer) 아키텍처 기반의 멀티태스크 딥러닝 추천 시스템으로 교체하는 것이 목표였다. 12개 태스크를 7개 전문가 네트워크가 처리하며, 태스크 간 관계를 명시적으로 모델링하는 구조다.
...
=== NEAS (Neglected-Expert Auxiliary Supervision) --- 첫 AUC-Positive Finding

*배경*: BRP-detached가 primary representation을 건드리지 않는 구조적 해법으로 AUC를 baseline까지 끌어올렸지만 $Delta$ $approx$ 0. Paper 3의 *AUC 자체의 positive* 주장을 위해서는 다른 축의 mechanism이 필요했다. 브레인스토밍 중 떠오른 아이디어 --- "게이트가 softmax로 expert 선호를 뚜렷하게 하면 무시된 expert로부터의 신호를 역가중치로 학습에 쓰면 어떨까?" --- 에서 출발. 세 해석 중 *load-balancing auxiliary supervision* (expert collapse 방지 regularizer) 방향을 선택했다.

*설계 (NEAS)*: 마지막 CGC 층의 `gate_weights` 와 `all_outs` 를 실시간으로 저장, per-task 역-게이트 집계 $"inv_agg"_t = sum (1 - g_t) / "sum" dot "all_outs"$ 를 계산. 이 집계 위에 per-task 보조 MLP head (128-64-output) 를 얹어 primary target $y$ 를 예측하도록 학습. 보조 loss는 primary loss 계산 방식 그대로(pos_weight, class_weight 재사용)하되 aux_weight=0.05로 가산. 추론 시에는 *완전히 사용 안 함* --- 학습 regularizer. `NEASConfig` 추가, HP 플래그 `use_neas`, 신규 파라미터 0.17M.

*결과 (10 에폭, seed=42)*: `struct_13_neas` final AUC *0.6739* (CGC 0.6728 대비 $Delta = +0.0011$, *양수*). 10 epoch 내내 *monotone 상승* (ep5에서 baseline 크로스, 이후 단조). F1 macro 0.2019 ($+0.0017$), NDCG\@3 0.6896 ($+0.0076$), MAE 0.9605 (CGC 0.9601과 실질 동급). val_loss 25.73 --- 8개 시나리오 중 최저. Best AUC 0.6739 @ ep10 (overfit 없음, 여전히 상승 추세).

*Per-task 패턴 --- 균등 상승*: 11/13 task에서 CGC 이상. Binary 7개 중 6개 개선 ($+$0.0004 $tilde$ $+$0.0029), churn_signal tied, top_mcc_shift tiny loss. Multiclass는 nba_primary $+0.0056$ (의미 있는 개선), segment/next_mcc는 거의 무변화. Regression 3개 중 2개 미세 개선.

*BRP-detached와의 성격 차이*:
...
#table(
  ...
  inset: 6pt,
  table.header(
    [*특성*], [*NEAS*], [*BRP-detached*]
  ),
  [$Delta$ AUC aggregate], [*$+0.0011$ (positive)*], [$-0.0007$ (tied)],
  [기제], [Training-time load balance regularizer], [Output-space error boosting],
  [효과 패턴], [전 task 균등 소폭 개선], [Hard task 1개 대박 ($+$256% next_mcc)],
  [Trade-off], [거의 없음], [대부분 task small loss],
  [추가 파라미터], [0.17M (aux heads only)], [0.36M (residual_bank)],
  [Inference 오버헤드], [*0 (training-only)*], [있음 (residual expert forward)],
)

=== NEAS + BRP-detached 조합 --- Non-Additive

*배경*: NEAS와 BRP-detached가 서로 다른 축에서 positive이므로 조합 시 additive 효과를 기대할 수 있었다. 가설: NEAS의 uniform 소폭 개선 + BRP-detached의 next_mcc 대박이 한 model에서 동시 발생.

*설계*: `struct_13_neas_brp_detached` --- `use_neas=true` + `use_brp=true` + `brp_detach_input=true`. 두 mechanism 모두 activate, 구조 변경 없음.

*결과 (10 에폭)*: Final AUC *0.6722* ($Delta = -0.0006$), Best AUC 0.6733 @ ep7. *NEAS 단독의 $+0.0011$ 이득이 소실*. F1 0.2062, NDCG\@3 0.6864 --- 둘 다 BRP-detached 단독보다 낮음. Trajectory는 ep7 peak 후 ep8-10 oscillation (NEAS 단독의 monotone 상승 패턴 소실).

*Per-task 분석 --- Conflict 지점*: 조합에서 살아남은 것은 *BRP의 next_mcc 구원* ($+0.0250$, 단독 $+0.0256$과 거의 동일). NEAS의 *uniform binary 개선은 전부 사라짐*:
...
#table(
  ...
  inset: 6pt,
  table.header(
    [*task*], [*CGC*], [*NEAS*], [*BRP-d*], [*조합*]
  ),
  [will_acquire_deposits], [0.6534], [$+$0.0029], [$+$0.0002], [*$-$0.0005 (사라짐)*],
  [will_acquire_lending], [0.6549], [$+$0.0021], [$+$0.0004], [*$-$0.0000 (사라짐)*],
  [will_acquire_payments], [0.6923], [$+$0.0009], [$-$0.0017], [*$-$0.0000 (사라짐)*],
  [nba_primary], [0.1876], [$+$0.0056], [$-$0.0035], [*$-$0.0071 (두 쪽 모두보다 악화)*],
  [next_mcc], [0.0100], [$+$0.0007], [*$+$0.0256*], [*$+$0.0250 (BRP 유지)*],
  [top_mcc_shift], [0.6312], [$-$0.0002], [$-$0.0011], [*$-$0.0031 (additive 악화)*],
)

*Mechanism of conflict*: NEAS의 aux loss는 shared experts를 *"모든 expert가 모든 task에 쓸모 있도록"* 균등화하는 방향으로 gradient를 보낸다. BRP-detached는 primary loss를 통해 shared experts를 *"primary-supporting 최적점"* 에 유지시킨다. 두 방향이 shared experts에서 정면 충돌 --- NEAS는 generalist를 요구하고 BRP는 specialist를 유지하려 한다. 결과로 shared experts가 두 방향 사이에서 진동하며, NEAS의 regularization 이득은 소실되고 BRP의 residual bank 기반 error 보정만 부분적으로 생존한다.

*Paper 3 narrative의 최종 형태*: 9-way 결과가 세 영역을 명확히 그린다:
...
#table(
  ...
  [*실패 (5개)*], [Representation-additive fusion (adaTT / AdaTT-sp / M1 / ECEB / loss-adaTT)], [$-0.001$ $tilde$ $-0.008$],
  [*Positive (BRP-detached)*], [Output-space boosting + gradient isolation], [$-0.0007$ (tied, F1/NDCG 크게 상승)],
  [*Positive (NEAS)*], [Training-time load-balancing regularizer], [*$+0.0011$ (실제 positive)*],
  [*Non-additive*], [NEAS + BRP-detached 조합], [$-0.0006$ (NEAS 이득 소실)],
)

두 positive 영역은 *서로 다른 문제를 해결* --- NEAS는 expert collapse 방지, BRP-detached는 primary error 보정. 독립적 positive finding이지만 *stacking 불가*. 운영 관점에서는 목적별로 택일해야 한다:
- Aggregate AUC 개선 + 전 task 균등 상승 $arrow$ NEAS
- Hard task (near-random) 구원 $arrow$ BRP-detached

이 non-additive 결과 자체가 Paper 3의 부가적 교훈이다: *"서로 다른 축의 positive mechanism을 stack해도 gain이 합쳐지지 않는다"* --- shared representation이 한정 자원이기 때문에.
...
"왜 모든 고객을 Lance에 담느냐?" 라는 직설적 이의 제기를 스스로에게 던지고 나서야 진단이 명확해졌다. LanceDB의 가치는 *결과가 붙은 케이스를 시간에 따라 누적*하는 것이지, 인구 전체의 현재 피처 상태를 스냅샷으로 미러링하는 것이 아니다. 올바른 단위는 "추천 케이스 1건 = 추론 로그 1건"이었다.

재설계는 세 가지로 정리되었다. 첫째, `recommendation_cases` 테이블을 Lambda 호출마다 기록한다 --- user_id, timestamp, 12개 태스크 확률, L1 reasons, FDTVS 점수. 둘째, DiagnosticCaseStore와 TemporalFactStore가 동일한 LanceDB 인스턴스를 공유하도록 통합하여 신규 DB 의존성을 추가하지 않았다. 셋째, 원본 피처 행렬 덤프는 제거했다 --- 필요 시 원본 parquet에서 재계산하면 된다.
...
온프렘 시스템은 규제 정합 관점에서 금융분야 AI 위험관리 프레임워크(AI RMF) 24개 항목 중 85%(11개 완전 + 9개 부분)에 대응하는 모듈을 자체 평가했으며, 미달 항목은 기술적 구현이 아닌 조직적 의사결정 사항이었다. 12개 규제 준수 모듈(AI 고지, 거부권, 인적 재처리, 공정성 모니터링, 이해충돌 방지, 쏠림 탐지, 프롬프트 인젝션 방어, 안전성 문서, 모델 카드, 감사 추적, 동의 관리, 품질 모니터링)이 구현되었다.
...
이 전제를 깨뜨린 건 다음 한마디였다. "2 PASS + 1 WARN, 이것도 마이너리티 리포트로 올려야지." 다수결로 WARN을 삼켜버리면, 규제 보고 시점에 "왜 그때 경고가 묻혔는가?"에 답할 수 없다. 이견은 흡수되는 것이 아니라 *반드시 지표면으로 올라와야* 한다.
...
트리거는 단순한 질문 두 줄이었다. "Paper 2 §5.10 했냐?" 와 "운영/감사 에이전트들도 랜스디비 참조하는 로직 구성되어있어?" 를 확인하기 위해 감사를 수행하자 *연결되지 않은 7개 피처*가 드러났다. 해결은 단일 커밋 `feat: Connect all 7 unlinked features to Lambda + agents`.
