...
AIOps 추천 시스템은 12개 태스크를 동시 예측해야 한다.
...
12개 태스크가 Shared Expert 파라미터를 공유할 때, gradient 충돌로 인한
...
진단 API: `detect_negative_transfer()`가 `{"churn_signal": ["will_acquire_lending", "nba_primary"]}` 형태로
...
#styled-table(
  ...
  [Lifecycle], [churn\_signal, product\_stability], [0.7], [0.3], [고객 생애주기],
...
12개 태스크의 가중치를 수동 튜닝하는 조합 폭발을 *자동 균형*으로 대체한다.
...
#styled-table(
  ...
  [12 task gradients], [$tilde$0.3x], [각 gradient는 shared\_param\_size],
