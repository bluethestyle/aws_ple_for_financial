...
AIOps 추천 시스템은 13개 태스크를 동시 예측해야 한다.
...
13개 태스크가 Shared Expert 파라미터를 공유할 때, gradient 충돌로 인한
...
진단 API: `detect_negative_transfer()`가 `{"churn_signal": ["has_nba", "nba_primary"]}` 형태로
...
도메인 지식에 기반하여 13개 태스크를 4개 그룹으로 분류한다:

#styled-table(
  (1fr, 2.5fr, 0.7fr, 0.7fr, 1.8fr),
  [*그룹*], [*멤버*], [*Intra*], [*Inter*], [*비즈니스 의미*],
  [Engagement], [cross\_sell\_count,\ will\_acquire\_deposits, will\_acquire\_investments,\ will\_acquire\_accounts, will\_acquire\_lending,\ will\_acquire\_payments], [0.8], [0.3], [고객 참여/전환],
  [Lifecycle], [churn\_signal, product\_stability,\ segment\_prediction], [0.7], [0.3], [고객 생애주기],
  [Value], [nba\_primary], [0.6], [0.3], [고객 가치/행동 패턴],
...
13개 태스크의 가중치를 수동 튜닝하는 조합 폭발을 *자동 균형*으로 대체한다.
...
#styled-table(
  ...
  [13 task gradients], [$tilde$0.3x], [각 gradient는 shared\_param\_size],
...
SageMaker `g4dn.xlarge` (16 GB RAM) 에서 1M 행 $times$ 1{,}210 컬럼 데이터 로딩 중 OOM 이 재현되었다. 원인을 추적한 결과, training-ready parquet 의 풀 materialize 와 fp16 round-trip 두 경로에서 메모리가 중복 점유되고 있었다.

=== 이전 (v13 까지)

`pq.read_table()` 로 parquet 전체를 Arrow Table 로 풀 materialize. 1M 행 $times$ 1{,}210 컬럼 fp64 = 9.68 GB Arrow. 이후 `_arrow_table_to_tensor` 에서 fp32 $arrow$ fp64 $arrow$ fp32 round-trip 과 `np.column_stack` copy 가 발생하여 peak 가 16 GB 를 돌파, 컨테이너가 oom-kill 되었다.
...
#styled-table(
  (1.6fr, 2.6fr),
  table.header([*수정*], [*효과*]),
  [DuckDB streaming `SELECT` + FLOAT cast (`containers/training/train.py:162-225`)], [9.68 GB $arrow$ 4.99 GB Arrow. CLAUDE.md §3.3 의 "DuckDB 우선" 정책 준수.],
  [`table.slice()` (zero-copy view) for contiguous splits], [`take()` 의 row-copy 를 회피. train/val/test 분리 시 메모리 추가 점유 없음.],
  [`_arrow_table_to_tensor` 의 fp32 $arrow$ fp64 $arrow$ fp32 round-trip 제거 + `column_stack` copy 제거], [Peak 16 GB $arrow$ $tilde$5 GB. fp32 그대로 텐서로 zero-copy 변환.],
)

=== 검증

`g4dn.xlarge` 에서는 여전히 OOM margin 이 좁아 최종적으로 `g4dn.2xlarge` (32 GB) 로 안착. peak RSS 는 $tilde$8 GB 수준에서 안정. `pd.read_parquet` 직접 사용 금지 원칙(CLAUDE.md §3.3)과도 정합.

#note[Arrow $arrow$ Tensor zero-copy 의 조건][
