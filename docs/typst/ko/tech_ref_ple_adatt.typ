...
AIOps 추천 시스템은 14개 태스크를 동시 예측해야 한다.
...
14개 태스크가 Shared Expert 파라미터를 공유할 때, gradient 충돌로 인한
...
도메인 지식에 기반하여 14개 태스크를 4개 그룹으로 분류한다:

#styled-table(
  (1fr, 2.5fr, 0.7fr, 0.7fr, 1.8fr),
  [*그룹*], [*멤버*], [*Intra*], [*Inter*], [*비즈니스 의미*],
  [Engagement], [has\_nba, cross\_sell\_count,\ will\_acquire\_deposits, will\_acquire\_investments,\ will\_acquire\_accounts, will\_acquire\_lending,\ will\_acquire\_payments], [0.8], [0.3], [고객 참여/전환],
...
14개 태스크의 가중치를 수동 튜닝하는 조합 폭발을 *자동 균형*으로 대체한다.
...
#styled-table(
  ...
  [14 task gradients], [$tilde$0.3x], [각 gradient는 shared\_param\_size],
