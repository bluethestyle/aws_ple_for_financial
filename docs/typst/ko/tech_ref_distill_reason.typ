...
#block(
  ...
  - 피처 선택: LGBM gain importance 기반 1205D → task별 ~280D (17 feature groups, 2026-04-28)
...
교사 IG는 941K 고객 × 1205D 피쳐 스케일에서 전체 PLE 그래프에 대한 역전파를 요구하여 OOM 장애를 유발한다.
...
_아래 차원 수치는 풀뱅크 설계(734D) 기준입니다. Santander 17-group 구현은 1205D 입력입니다._

*풀뱅크 설계 기준:* 734D(설계) / 1205D (Santander 17-group 구현) $arrow$ ~140D(Stage 1) $arrow$ 최종 LGBM 입력

*Stage 1 -- LGBM Gain Importance 필터:*
누적 gain importance 기준 상위 95%를 포착하는 상위-$k$ 피쳐를 태스크별로 유지한다.
결과 피쳐 집합은 일반적으로 태스크당 40~80개 피쳐이다(전체 입력 차원에서 축소).
...
#table(
  ...
  [3], [LGBM gain importance 피쳐 선택 (1205D $arrow$ task별 ~140D)],
...
이는 단순 기준선보다 나쁜 증류 모델의 배포를 방지하여, 가이드라인 거버넌스 원칙 §1.4(출시 전 위험경감 조치 검증)에 대응합니다.
...
PLE-adaTT는 피쳐 벡터(풀뱅크 설계: 734D; 현재 Santander 17-group 구현: 1205D)를 소비하여 확률 점수를 출력하지만
    ...
    |-> LGBM Student (태스크별, 200D 피쳐) + Platt 캘리브레이션 (확률 중요 태스크)
      |-> ONNX Export (ZipMap 제거)
        |-> Lambda FallbackRouter
            ...
            |-> 3계층: 규칙 엔진 (12개 태스크, Financial DNA 라우팅)
...
#table(
  ...
  [3계층: 규칙 엔진], [Python 규칙 셋: 12개 태스크별 규칙 + Financial DNA 피쳐 라우팅], [`scores` (휴리스틱), `contributing_features` (규칙 기반)],
)
...
규칙 엔진은 Financial DNA 피쳐 라우팅을 중심으로 구성된 12개 태스크별 규칙을 구현한다.

- *DNA 라우팅:* 항상소득 고객($"CV" < 0.2$) → 장기 상품 규칙; 일시소득 고객($"CV" >= 0.5$) → 단기·저관여 규칙
- *태스크 규칙:* 12개 태스크 각각에 3--5개 해석 가능한 피쳐 기반 휴리스틱 점수 함수 (예: 이탈 규칙은 최신성 + 빈도 + 고객지원 통화 수 사용)
