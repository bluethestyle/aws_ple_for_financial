...
#block(
  ...
  본 문서는 *풀뱅크 설계(734D)*를 기준으로 작성되었습니다. 현재 Santander 벤치마크 구현은 *350D (13 feature groups)*입니다. 본문의 차원 수치(734D, 200D, 140D 등)는 풀뱅크 설계 기준이며, 실제 Santander 구현에서는 중간 차원이 달라질 수 있습니다. 실제 구현의 차원 명세는 `outputs/phase0/feature_schema.json`을 참조하십시오.
]
...
#table(
  ...
  [피쳐 차원], [734D(설계) / 350D(구현)], [200D (IG 선택 후, 설계 기준)],
...
#note[LGBM Student의 Temperature][
...
_아래 차원 수치는 풀뱅크 설계(734D) 기준입니다. Santander 구현(350D)에서는 중간 차원이 달라질 수 있습니다._

*풀뱅크 설계 기준:* 734D(설계) / 350D(구현) $arrow$ 200D(Stage 1) $arrow$ ~140D(Stage 2) $arrow$ 최종 LGBM 입력
