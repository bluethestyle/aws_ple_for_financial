...
#v(12pt)
#warn[설계 vs 구현 차원 안내][
  본 문서는 *풀뱅크 설계(734D)*를 기준으로 작성되었습니다. 현재 Santander 벤치마크 구현은 *~349D 입력 / Phase 0 후 403D (13 feature groups)*입니다. 실제 구현의 차원 명세는 `outputs/phase0/feature_schema.json`을 참조하십시오. 부록 "설계 vs 구현 차원 매핑"에서 각 그룹별 차이를 확인할 수 있습니다.
]
...
아래 표는 풀뱅크 설계(734D) 기준이다. 현재 Santander 벤치마크 구현은 ~349D 입력 / Phase 0 후 403D (13 feature groups)이며,
...
#styled-table(
  ...
  [*합계 (normalized)*], [*644D (설계) / ~349D (구현 입력)*], [],
  [Raw power-law copy], [90D], [멱법칙 컬럼의 log1p 원본 (스케일링 미적용); Phase 0에서 추가되어 403D로 확장],
  [*Main Tensor 총합*], [*734D (설계) / ~349D 입력 → 403D (Phase 0 후 구현)*], [설계: 644D normalized + 90D raw; 구현: ~349D 입력, Phase 0 변환 후 403D],
)
...
#warn[참고][이 부록은 풀뱅크 설계(734D)와 현재 Santander 벤치마크 구현(~349D 입력 / Phase 0 후 403D, Phase 0 v3/v4) 간의 차원 차이를 정리한 것입니다. 구현 차원은 `outputs/phase0/feature_schema.json`에서 확인할 수 있습니다. *FeatureRouter 활성화*: ~349D 입력은 전문가별로 서브셋 슬라이싱되어 전달됩니다 — 각 전문가의 실제 입력 차원은 아래 per-expert 컬럼을 참조하십시오.]

#styled-table(
  (1.2fr, 0.8fr, 0.8fr, 1.2fr, 2fr),
  table.header([*피처 그룹*], [*설계 (734D)*], [*구현 (~349D 입력 / 403D)*], [*주요 수신 전문가*], [*비고*]),
  ...
  [*합계*], [*734D*], [*~349D 입력 / 403D (Phase 0 후)*], [—], [13 feature groups; 이종 입력으로 각 전문가에 분배],
)
