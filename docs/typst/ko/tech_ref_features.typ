...
#v(12pt)
#warn[설계 vs 구현 차원 안내][
  본 문서는 *풀뱅크 설계(734D)*를 기준으로 작성되었습니다. 현재 Santander 벤치마크 구현은 *1211D 입력 (17 feature groups)*입니다. 실제 구현의 차원 명세는 `outputs/phase0/feature_schema.json`을 참조하십시오. 부록 "설계 vs 구현 차원 매핑"에서 각 그룹별 차이를 확인할 수 있습니다.
]
...
+ *예측 입력*: PLE-adaTT 모델의 입력 텐서로서 13개 태스크의 예측에 기여한다.
...
전체 1205D 입력 텐서(1211D, 17 groups)는 `feature_groups.yaml`의 `target_experts` 선언에 따라
`FeatureRouter`가 전문가별 지정 인덱스를 슬라이싱하여 전달한다.
각 전문가는 전체 피처가 아닌 관련 서브셋만 수신한다:
deepfm=977D, temporal\_ensemble=116D, hgcn=58D, perslay=32D, causal=129D, lightgcn=955D, optimal\_transport=95D (mlp=57D 는 PLE CGC 의 task-specific tower).
모델 파라미터는 ~2.8M (post-FeatureRouter pruning; lag tensor 800D 라우팅으로 그룹 토글에 따라 변동).

== 전체 피처 텐서 구성

아래 표는 풀뱅크 설계(734D) 기준이다. 현재 Santander 벤치마크 구현은 1211D 입력 / 17 feature groups이며,
...
#styled-table(
  ...
  [*합계 (normalized)*], [*644D (설계) / 1205D (구현 입력)*], [],
  [Raw power-law copy], [90D], [멱법칙 컬럼의 log1p 원본 (스케일링 미적용); 설계 기준; 구현에서는 별도 log1p 복사본 미포함],
  [*Main Tensor 총합*], [*734D (설계) / 1211D 입력 (구현)*], [설계: 644D normalized + 90D raw; 구현: 1211D (17 groups)],
)
...
#note[FeatureRouter 슬라이싱][
  구현에서 1205D 입력 텐서(1211D, 17 groups)는 `FeatureRouter`를 통해 전문가별로 슬라이싱된다.
  각 전문가는 `feature_groups.yaml`의 `target_experts` 필드에 지정된 피처 그룹의 인덱스만 수신한다.
  전문가별 입력 차원: deepfm=977D, temporal\_ensemble=116D, hgcn=58D, perslay=32D, causal=129D, lightgcn=955D, optimal\_transport=95D (mlp=57D 는 PLE CGC 의 task-specific tower).
...
#warn[참고][이 부록은 풀뱅크 설계(734D)와 현재 Santander 벤치마크 구현(1211D 입력, 17 feature groups) 간의 차원 차이를 정리한 것입니다. 구현 차원은 `outputs/phase0/feature_schema.json`에서 확인할 수 있습니다. *FeatureRouter 활성화*: 1205D 입력은 전문가별로 서브셋 슬라이싱되어 전달됩니다 — 각 전문가의 실제 입력 차원은 아래 per-expert 컬럼을 참조하십시오.]

#styled-table(
  (1.2fr, 0.8fr, 0.8fr, 1.2fr, 2fr),
  table.header([*피처 그룹*], [*설계 (734D)*], [*구현 (1205D)*], [*주요 수신 전문가*], [*비고*]),
  [TDA], [70D], [32D], [perslay (32D)], [tda\_global 16D + tda\_local 16D],
  [HMM], [48D + 5D (별도)], [25D], [temporal 일부], [main tensor only],
  [Base (Profile 등)], [238D], [53D], [deepfm, causal, ot 일부], [demographics 11D + product\_holdings 24D + derived\_temporal 4D + txn\_behavior 14D],
  [Graph], [미명시], [100D], [lightgcn (955D 포함)], [graph\_collaborative 66D + product\_hierarchy 34D],
  [Merchant / Hierarchy], [27D], [27D], [hgcn (58D 포함)], [MCC Poincaré 임베딩 (merchant\_hierarchy)],
  [GMM + Model], [22D], [49D], [deepfm, causal 일부], [gmm\_clustering 22D + model\_derived 27D],
  [Lag Tensor (신규)], [—], [800D], [deepfm, lightgcn], [txn\_lag\_tensor: K=200×4 (lag\_extractor)],
  [Rolling Stats (신규)], [—], [20D], [deepfm, causal, ot], [txn\_rolling\_stats: 4 windows×5 (rolling\_stats\_extractor)],
  [Multi-hot (신규)], [—], [55D], [lightgcn, deepfm, hgcn], [nba\_label\_multihot 24D + mcc\_top30\_multihot 31D (topn\_multihot\_extractor)],
  [Mamba], [—], [50D], [temporal\_ensemble], [GPU 사전 계산, cached parquet DuckDB JOIN],
  [기타 (Economics, SIR 등)], [335D], [—], [—], [Santander 벤치마크 미구현],
  [*합계*], [*734D*], [*1211D*], [—], [17 feature groups],
)

#note[per-expert 입력 차원 요약 (FeatureRouter 활성)][
  deepfm=977D, temporal\_ensemble=116D, hgcn=58D, perslay=32D, causal=129D, lightgcn=955D, optimal\_transport=95D (mlp=57D 는 PLE CGC 의 task-specific tower).
  출력은 모두 64D로 균일. 모델 파라미터: 4.77M → ~2.8M (post-FeatureRouter pruning; lag tensor 800D로 deepfm/lightgcn 대폭 증가, 파라미터 추정치는 그룹 토글에 따라 변동).
...
- *경제학 계열*: Friedman 1957 (_Consumption Function_); Hall 1978 (PIH); Hodrick & Prescott 1997; Kalman 1960; Kahneman & Tversky 1979 (Prospect Theory).
