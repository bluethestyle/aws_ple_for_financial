...
`FeatureRouter`는 *현재 완전히 활성화*되어 있으며, `build_model()` 시점에 `feature_groups.yaml`의 `target_experts` 선언으로부터 자동 생성된다. 전체 ~349D 입력 피처 텐서(Phase 0 후 403D)를 expert별로 슬라이싱하여 *이종 입력 차원*을 제공한다. expert_routing은 그룹 단위로 동작하며 하드코딩 라우팅은 없다.

#table(
  columns: (auto, auto, 1fr),
  align: (left, center, left),
  table.header[*Expert*][*입력 차원*][*라우팅된 피처 그룹 (요약)*],
  [`deepfm`], [168D], [demographics, product\_holdings, txn\_behavior, gmm\_clustering, model\_derived 등],
  [`temporal_ensemble`], [139D], [txn\_behavior, derived\_temporal, hmm\_states, mamba\_temporal 등],
  [`hgcn`], [27D], [merchant\_hierarchy (MCC Poincaré 임베딩)],
  [`perslay`], [32D], [tda\_local (16D) + tda\_global (16D)],
  [`causal`], [161D], [demographics, product\_holdings, txn\_behavior, derived\_temporal, model\_derived 등],
  [`lightgcn`], [100D], [product\_hierarchy (32D) + graph\_collaborative (66D) + 기타],
  [`optimal_transport`], [127D], [txn\_behavior, gmm\_clustering, model\_derived 등],
  [`mlp` (task expert)], [가변], [라우팅된 피처 서브셋],
)

FeatureRouter 활성화 효과: 총 파라미터 4.77M → *~2.8M* (감소). `dim_normalize=True` 설정으로 expert 출력이 공통 `output_dim`(64D)으로 투영된 후 CGC gate에 입력되므로, gate 연산은 입력 차원 이종성에 무관하다. HGCN은 merchant_hierarchy(27D MCC Poincaré), LightGCN은 product_hierarchy(32D) + graph_collaborative(66D) = 98D를 주 입력으로 받는다.
...
== 13개 Feature Group (~349D 입력 / Phase 0 후 403D)
...
```
5-Axis Features (~349D input / 403D post-Phase 0)
    │
    ▼ FeatureRouter (축별 분배)
┌──────────────────────────────────┐
