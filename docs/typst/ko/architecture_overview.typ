...
`FeatureRouter`는 *현재 완전히 활성화*되어 있으며, `build_model()` 시점에 `feature_groups.yaml`의 `target_experts` 선언으로부터 자동 생성된다. 전체 1205D 입력 피처 텐서(1211D, 17 groups)를 expert별로 슬라이싱하여 *이종 입력 차원*을 제공한다. expert_routing은 그룹 단위로 동작하며 하드코딩 라우팅은 없다.

#table(
  columns: (auto, auto, 1fr),
  align: (left, center, left),
  table.header[*Expert*][*입력 차원*][*라우팅된 피처 그룹 (요약)*],
  [`deepfm`], [977D], [demographics, product\_holdings, txn\_behavior, derived\_temporal, gmm\_clustering, model\_derived, txn\_lag\_tensor(800D), txn\_rolling\_stats, nba\_label\_multihot, mcc\_top30\_multihot],
  [`temporal_ensemble`], [116D], [txn\_behavior (14D), hmm\_states (25D), mamba\_temporal (50D), model\_derived (27D)],
  [`hgcn`], [58D], [merchant\_hierarchy (27D) + mcc\_top30\_multihot (31D)],
  [`perslay`], [32D], [tda\_global (16D) + tda\_local (16D)],
  [`causal`], [129D], [demographics, product\_holdings, txn\_behavior, derived\_temporal, product\_hierarchy, gmm\_clustering, txn\_rolling\_stats],
  [`lightgcn`], [955D], [product\_hierarchy (34D) + graph\_collaborative (66D) + txn\_lag\_tensor (800D) + nba\_label\_multihot (24D) + mcc\_top30\_multihot (31D)],
  [`optimal_transport`], [95D], [demographics, product\_holdings, txn\_behavior, derived\_temporal, gmm\_clustering, txn\_rolling\_stats],
  [`mlp` (task expert)], [57D], [demographics (11D) + product\_holdings (24D) + gmm\_clustering (22D)],
)

FeatureRouter 활성화 효과: 총 파라미터 4.77M → *~2.8M* (post-FeatureRouter pruning; lag tensor 800D가 deepfm/lightgcn에 라우팅되어 파라미터 추정치는 그룹 토글에 따라 변동). `dim_normalize=True` 설정으로 expert 출력이 공통 `output_dim`(64D)으로 투영된 후 CGC gate에 입력되므로, gate 연산은 입력 차원 이종성에 무관하다. HGCN은 merchant\_hierarchy(27D) + mcc\_top30\_multihot(31D) = 58D, LightGCN은 product\_hierarchy(34D) + graph\_collaborative(66D) + txn\_lag\_tensor(800D) + nba\_label\_multihot(24D) + mcc\_top30\_multihot(31D) = 955D를 입력으로 받는다.
...
초기 설계에서는 GMM 클러스터별 태스크 서브헤드를 두려 했으나, K=20, T=12이면 서브헤드 240개 → 관리 불가, 과적합 위험. 방향을 전환하여 *태스크를 금융적 DNA 관점으로 그룹핑*했다:
...
#table(
  ...
  [lifecycle], [고객이 어디에 있는가], [churn\_signal, product\_stability], [0.7], [0.3],
...
== 17개 Feature Group (1211D 입력)

=== 4개 Base Group (transform 타입)

기존 raw 컬럼을 변환하여 생성. Generator가 필요 없다.
- `demographics`: 인구통계 피처 11D
- `product_holdings`: 상품 보유 현황 24D
- `txn_behavior`: 거래 행동 지표 14D
- `derived_temporal`: 파생 시간 집계 4D

=== 13개 Generated Group (generate 타입)
...
#table(
  ...
  [tda\_global], [tda\_global], [16D], [모집단 수준 위상 분석 (H0+H1, 365일 윈도우)],
  [tda\_local], [tda\_local], [16D], [개인 수준 위상 분석 (H0+H1, 90일 윈도우)],
  [hmm\_states], [hmm], [25D], [journey/lifecycle/behavior 3중 모드 상태 추정],
  [mamba\_temporal], [mamba], [50D], [Selective SSM (GPU 사전 계산, cached parquet)],
  [product\_hierarchy], [graph], [34D], [24개 상품 Poincaré 임베딩 (6 L1 그룹)],
  [merchant\_hierarchy], [merchant\_hierarchy], [27D], [MCC L1→L2 트리 Poincaré 임베딩 (HGCN 입력)],
  [graph\_collaborative], [graph], [66D], [고객-상품 이분 그래프 LightGCN 임베딩],
  [gmm\_clustering], [gmm], [22D], [Soft posterior probabilities (20 클러스터)],
  [model\_derived], [model\_features], [27D], [KMeans(5) + Bandit/MAB(4) + LNN(18)],
  [txn\_lag\_tensor], [lag\_extractor], [800D], [우측 정렬 lag 평탄화: K=200×4 시퀀스 피처],
  [txn\_rolling\_stats], [rolling\_stats\_extractor], [20D], [4 윈도우(7/30/90/180일)×5 지표],
  [nba\_label\_multihot], [topn\_multihot\_extractor], [24D], [고정 어휘 24개 상품 multi-hot (NBA 추천셋)],
  [mcc\_top30\_multihot], [topn\_multihot\_extractor], [31D], [Top-30 MCC multi-hot + others 컬럼],
)
...
#table(
  ...
  [5], [LabelDeriver], [Config-driven 12개 레이블 생성], [---],
...
== Phase 1--3: Ablation Study (23 시나리오, 총 48개 실험 구성)
...
#figure(
  placement: auto,
  {
    ...
    fletcher.diagram(
      ...
      node((0, 7), [*Stage 5: LabelDeriver* #text(size: 6pt)[(12 tasks, config-driven)]], width: 55mm, fill: proc-fill, name: <s5>),
      ...
      node((0, 10), [#text(size: 7pt, fill: luma(100))[▼ Phase 1--5 (다음 그림 참조)]], width: 60mm, fill: luma(250), name: <cont>),
      edge(<s6>, <cont>, "->", stroke: 0.4pt + luma(150)),
    )
  },
  caption: [End-to-End 파이프라인 (1/2): Phase 0 데이터 준비 — S3 원본 수집부터 피처/레이블/시퀀스 텐서 저장까지.],
)
...
#figure(
  placement: auto,
  {
    ...
    fletcher.diagram(
      ...
      // Continuation note
      node((0, 0), [#text(size: 7pt, fill: luma(100))[▲ Phase 0 출력 (sequences.npy 등)]], width: 60mm, fill: luma(250), name: <prev>),
      ...
      node((0, 9), [*Stage 10: CPE + Agentic Reason Orchestrator* \ #text(size: 6pt)[FD-TVS scoring + DNA modifier · Constraint Engine] \ #text(size: 6pt)[L1+L2a+L2b 추론 체인]], width: 55mm, fill: proc-fill, name: <s10>),
      node((0, 10), [*Lambda / ECS Fargate (서버리스 서빙)* \ #text(size: 6pt)[추천 상품 + 자연어 추천사유 + 불확실성 정량화]], width: 55mm, fill: out-fill, name: <serving>),
  ...
  caption: [End-to-End 파이프라인 (2/2): Phase 1--3 학습 → Phase 4 증류 → Phase 5 서빙.],
)

== 모델 내부 데이터 흐름

```
5-Axis Features (1211D, 17 groups)
    │
    ▼ FeatureRouter (축별 분배)
┌──────────────────────────────────┐
│  Expert Basket (7 shared)        │
│  DeepFM  ← 977D (State+Lag)     │
│  Temporal ← 116D (Timeseries)   │
│  HGCN   ← 58D (Hierarchy+MCC)  │
│  PersLay ← 32D (Snapshot/TDA)  │
│  Causal  ← 129D (Snapshot)      │
│  LightGCN← 955D (Item+Lag)     │
│  OT     ← 95D (Snapshot)        │
└──────────┬───────────────────────┘
           │
...
- `tasks`: 12개 태스크 (name, type, loss, loss\_weight, label\_col)
...
*`feature_groups.yaml`*: 17개 피처 그룹 정의
...
파이프라인 전체를 비동기로 감시하는 2개의 자율 진단 에이전트가 별도 계층으로 작동한다:
- *OpsAgent*: 7개 체크포인트(인제스천~A/B테스트) 모니터링, 연쇄 영향 분석
- *AuditAgent*: 5개 관점(공정성, 집중도, 추천사유 품질, 규제 적합성, 데이터 계보) 감사
...
#table(
  ...
  [DynamoDB 테이블], [PipelineState (작업 재개 상태), reason_cache (L2a 캐시), audit 로그 테이블들],
  [LanceDB], [DiagnosticCaseStore (감사 이력) + TemporalFactStore + recommendation_cases (단일 인스턴스 공유)],
...
온프레미스(폐쇄망) 환경에서는 동일한 파이프라인이 로컬 GPU(RTX 4070)에서 실행되며, Bedrock 대신 Exaone 3.5 7.8B(사유 생성)와 Qwen 2.5 14B Q4(에이전트 합의)를 사용한다. 데이터가 외부로 전송되지 않으므로 데이터 보호가 구조적으로 보장된다.
