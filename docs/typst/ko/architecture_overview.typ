...
`FeatureRouter`는 *현재 완전히 활성화*되어 있으며, `build_model()` 시점에 `feature_groups.yaml`의 `target_experts` 선언으로부터 자동 생성된다. 전체 350D 피처 텐서를 expert별로 슬라이싱하여 *이종 입력 차원*을 제공한다. expert_routing은 그룹 단위로 동작하며 하드코딩 라우팅은 없다.
...
#table(
  ...
  [5], [LabelDeriver], [Config-driven 14개 레이블 생성], [---],
