...
_(풀뱅크 설계: 644D. Santander 17-group 구현: Causal expert는 FeatureRouter를 통해 129D를 수신; 실제 compressor 입력 차원은 129.)_

원시 피처를 한꺼번에 인과 분석하면 풀뱅크 규모에서 $644^2 approx 41$만 개, Santander 구현에서도 $129^2 approx 1.7$만 개 쌍 관계가 발생한다.
...
구성: `Linear(644, 128)` $arrow$ `LayerNorm(128)` $arrow$ `SiLU` $arrow$ `Dropout(0.2)` $arrow$ `Linear(128, 32)` $arrow$ `softmax`. _(풀뱅크 설계: 644D 입력. Santander 17-group: OT expert는 FeatureRouter를 통해 95D 수신.)_
...
#table(
  ...
  [입력 차원], [129D (demographics 11D + product\_holdings 24D + txn\_behavior 14D + derived\_temporal 4D + product\_hierarchy 34D + gmm 22D + rolling\_stats 20D)], [95D (demographics 11D + product\_holdings 24D + txn\_behavior 14D + derived\_temporal 4D + gmm 22D + rolling\_stats 20D)],
...
DeepFM, Causal, OT 세 Expert는 각각 서로 다른 피처 서브셋을 입력받지만(Causal: 129D, OT: 95D, 17-group Santander 구현 기준, FeatureRouter를 통한 슬라이싱), 추출하는 수학적 구조는 근본적으로 다르다:
