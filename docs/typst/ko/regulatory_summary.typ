...
규제 준수의 실질적 집행은 2개의 자율 진단 에이전트가 담당한다. OpsAgent는 파이프라인 성능/안정성을, AuditAgent는 공정성/규제 적합성/추천사유 품질을 감사한다. 3-에이전트 합의와 마이너리티 리포트 보존으로 할루시네이션 리스크를 완화하며, 모든 진단 결과는 케이스 스토어에 축적되어 규제 감사 증적으로 활용된다.
