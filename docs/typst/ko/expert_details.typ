...
#block(
  ...
  - 7개 이질적 전문가: DeepFM, Temporal Ensemble, Hyperbolic GCN, PersLay/TDA, LightGCN, Causal/NOTEARS, Optimal Transport
...
#block(
  ...
  현재 Santander 벤치마크 구현은 1211D 입력 (17 feature groups)입니다.
  *FeatureRouter 활성화*: 각 전문가는 전체 1205D를 모두 받는 것이 아니라
  `feature_groups.yaml`의 `target_experts` 선언에 따라 서브셋만 입력받습니다.
  전문가별 실제 입력 차원: deepfm=977D, temporal=116D, hgcn=58D, perslay=32D,
  causal=129D, lightgcn=955D, ot=95D, mlp=57D. 모델 파라미터: ~2.8M (post-FeatureRouter pruning; 그룹 토글에 따라 변동).
  HGCN은 merchant\_hierarchy(27D) + mcc\_top30\_multihot(31D) = 58D, LightGCN은 product\_hierarchy(34D) + graph\_collaborative(66D) + txn\_lag\_tensor(800D) + nba\_label\_multihot(24D) + mcc\_top30\_multihot(31D) = 955D 입력.
]
...
#figure(
  table(
    ...
    [*Input (현재 구현)*], [977D — FeatureRouter가 1205D에서 DeepFM 지정 서브셋 슬라이싱 (demographics 11D + product\_holdings 24D + txn\_behavior 14D + derived\_temporal 4D + gmm\_clustering 22D + model\_derived 27D + txn\_lag\_tensor 800D + txn\_rolling\_stats 20D + nba\_label\_multihot 24D + mcc\_top30\_multihot 31D)],
  ...
  caption: [DeepFM Expert 입출력 사양. FeatureRouter 활성화 시 입력 차원 977D (txn\_lag\_tensor 800D 포함), 출력 64D 동일.],
)
...
#figure(
  table(
    ...
    [FeatureRouter 입력], [955D (product\_hierarchy 34D + graph\_collaborative 66D + txn\_lag\_tensor 800D + nba\_label\_multihot 24D + mcc\_top30\_multihot 31D)], [58D (merchant\_hierarchy 27D + mcc\_top30\_multihot 31D)],
  ),
  caption: [Dual GCN 아키텍처 비교 (Phase 0 v3/v4). HGCN은 58D (merchant\_hierarchy 27D + mcc\_top30\_multihot 31D), LightGCN은 955D (product\_hierarchy 34D + graph\_collaborative 66D + txn\_lag\_tensor 800D + nba\_label\_multihot 24D + mcc\_top30\_multihot 31D) 입력을 받는다.],
)

=== HGCN 입력 상세 (27D merchant_hierarchy)

27D `merchant_hierarchy` 그룹은 Poincaré disk에 임베딩된 MCC L1→L2 트리 구조를 포착한다:

#table(
  columns: (auto, auto, 1fr),
  stroke: 0.5pt,
  [*서브그룹*], [*차원*], [*내용*],
  [L1 Poincaré 좌표], [4D], [Poincaré disk 내 L1 MCC 카테고리 위치],
  [L2 Poincaré 좌표], [4D], [L2 MCC 서브카테고리 위치],
  [Brand SVD], [8D], [브랜드 수준 SVD 임베딩],
  [집계 통계], [4D], [L1 버킷별 소비 비중, 빈도, 최근성],
  [Depth 피처], [3D], [계층 깊이 지표],
  [Spread 피처], [4D], [트리 레벨 간 MCC 사용 폭],
)

HGCN은 *쌍곡 공간(Poincaré disk)에서 MCC L1→L2 트리 구조를 학습*한다. LightGCN의 보완재 역할:
HGCN은 "가맹점 카테고리 간 계층 관계"를 학습하고, LightGCN은 "고객이 어떤 상품을 함께 보유하는가"(product co-holding 이분 그래프 CF)를 학습한다.

=== LightGCN 입력 상세 (955D)

LightGCN은 `product_hierarchy`(34D) + `graph_collaborative`(66D) + `txn_lag_tensor`(800D) + `nba_label_multihot`(24D) + `mcc_top30_multihot`(31D) = 총 955D를 입력받는다.
*상품-고객 이분 그래프*에서 협업 필터링을 수행하며, lag tensor(800D)가 그래프 구조를 보완하는 원시 시간 신호를 제공한다. HGCN의 가맹점 트리 중심 역할을 보완한다.
...
#figure(
  table(
    columns: (auto, auto),
    stroke: 0.5pt,
    [*Input*], [상품-고객 이분 그래프 (product co-holding) + 955D 피처 서브셋 (FeatureRouter: product\_hierarchy 34D + graph\_collaborative 66D + txn\_lag\_tensor 800D + nba\_label\_multihot 24D + mcc\_top30\_multihot 31D)],
  ...
  caption: [LightGCN Expert 입출력 사양. 피처 입력 955D (FeatureRouter 활성화: product\_hierarchy 34D + graph\_collaborative 66D + txn\_lag\_tensor 800D + nba\_label\_multihot 24D + mcc\_top30\_multihot 31D). 상품 co-holding 협업 필터링 담당; lag tensor가 그래프 구조에 원시 시간 신호 보완 — HGCN MCC 계층과 상호 보완.],
)
...
- LightGCN의 도메인은 *상품 co-holding* (고객이 어떤 상품을 함께 보유하는가)이며, MCC 트리 구조는 H-GCN의 도메인이다.
...
- *짧은 ablation에서의 warmup/freeze epoch 주의*: 3--5 epoch 단기 ablation 시 `warmup_epochs=1`, `freeze_epochs=3`으로 설정하여 dynamic phase에 진입을 보장한다. 기본값은 15+ epoch 설계로, 단기 실행 시 warmup phase에서만 끝날 수 있다.
...
// ============================================================
= Feature Engineering Overview --- 11 Disciplines, 1211D 입력 (17 그룹) <sec9-features>
// ============================================================
...
#block(
  ...
  *FeatureRouter와 피처 그룹 라우팅*
  ...
  전체 1205D 피처 텐서(1211D, 17 groups)는 모든 전문가에게 동일하게 전달되는 것이 아니다.
  `feature_groups.yaml`의 `target_experts` 선언에 따라 `FeatureRouter`가
  각 전문가에게 관련 서브셋만 슬라이싱하여 전달한다.
  예를 들어 PersLay는 TDA 피처 그룹(32D)만, LightGCN은 product\_hierarchy(34D) + graph\_collaborative(66D) + txn\_lag\_tensor(800D) + nba\_label\_multihot(24D) + mcc\_top30\_multihot(31D) = 955D를 입력받는다.
  전문가별 입력 차원: deepfm=977D, temporal\_ensemble=116D, hgcn=58D, perslay=32D,
  causal=129D, lightgcn=955D, optimal\_transport=95D (mlp=57D 는 PLE CGC 의 task-specific tower).
...
#figure(
  ...
  caption: [11개 학문 분야에서 도출된 피처 체계. Santander 구현: 1211D 입력 (17 feature groups). 신규 그룹: txn\_lag\_tensor 800D (lag\_extractor), txn\_rolling\_stats 20D (rolling\_stats\_extractor), nba\_label\_multihot 24D + mcc\_top30\_multihot 31D (topn\_multihot\_extractor). 이 표는 풀뱅크 개념 설계 기준; Santander 실제 차원은 feature\_groups.yaml 참조.],
)
...
전체 시스템의 1211D 입력 (17 feature groups) main tensor (Phase 0 v3/v4 구현 기준)은 5개 피처 축에 걸쳐 있다.
...
- *피처 상호작용*: Guo et al. IJCAI 2017 (DeepFM); Rendle ICDM 2010 (FM); Wang et al. KDD 2017, WWW 2021 (Deep Cross v1/v2).
- *시계열*: Gu & Dao NeurIPS 2023 (Mamba); Hasani et al. AAAI 2021 (LNN); Nie et al. ICLR 2023 (PatchTST).
...
- *인과 / NOTEARS / DAGMA*: Pearl 2009; Zheng et al. NeurIPS 2018; Bello et al. ICML 2022.
