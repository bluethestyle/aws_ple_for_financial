...
#block(
  ...
  - FeatureRouter: `feature_groups.yaml`의 `target_experts`로 서브셋 입력 (deepfm=168D, temporal=139D, hgcn=27D, perslay=32D, causal=161D, lightgcn=100D, ot=127D)
...
#block(
  ...
  *FeatureRouter 활성화*: 각 전문가는 전체 ~349D를 모두 받는 것이 아니라
  ...
  HGCN은 merchant_hierarchy(27D MCC Poincaré), LightGCN은 product_hierarchy(32D) + graph_collaborative(66D) = 98D 핵심 입력.
]
...
#figure(
  table(
    ...
    [FeatureRouter 입력], [100D (product\_hierarchy 32D + graph\_collaborative 66D + 기타)], [27D (merchant\_hierarchy)],
  ),
  caption: [Dual GCN 아키텍처 비교 (Phase 0 v3/v4). HGCN은 merchant_hierarchy(27D MCC Poincaré), LightGCN은 product_hierarchy(32D) + graph_collaborative(66D) = 98D 핵심 입력을 받는다.],
)
...
LightGCN은 `product_hierarchy`(32D) + `graph_collaborative`(66D) + 추가 아이템 축 피처 = 총 100D를 입력받는다.
*상품-고객 이분 그래프*에서 협업 필터링을 수행하며, HGCN의 가맹점 트리 중심 역할을 보완한다.
...
#figure(
  table(
    columns: (auto, auto),
    stroke: 0.5pt,
    [*Input*], [상품-고객 이분 그래프 (product co-holding) + 100D 피처 서브셋 (FeatureRouter: product\_hierarchy 32D + graph\_collaborative 66D + 아이템 축 그룹)],
  ...
  caption: [LightGCN Expert 입출력 사양. 피처 입력 100D (FeatureRouter 활성화: product_hierarchy 32D + graph_collaborative 66D). 상품 co-holding 협업 필터링 담당 — HGCN의 MCC 가맹점 계층과 상호 보완.],
)
...
#block(
  ...
  *FeatureRouter와 피처 그룹 라우팅*
  ...
  전문가별 입력 차원 (Phase 0 v3/v4): deepfm=168D, temporal=139D, hgcn=27D, perslay=32D,
  causal=161D, lightgcn=100D, ot=127D.
