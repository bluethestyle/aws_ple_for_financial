...
#card(title: "Art. 13 요구사항과 시스템 대응", accent: navy)[
  #table(
    ...
    [입력 데이터 사양 공개], [피처 스키마 자동 문서화 (1211D 모델 입력 / 403D Phase 0 후 raw 컬럼 / 734D 풀뱅크 설계)\
    학습 데이터 출처·범위·가명처리 여부 기록],
    [성능 수준 공개], [모델 카드 자동 생성 (아키텍처·성능·피처 중요도)\
    태스크별 독립 AUC 추적 (12개 태스크)],
...
#table(
  ...
  [3계층 (Safety Net)], [규칙 기반 엔진: 12개 태스크 규칙 + Financial DNA 라우팅], [두 모델 계층 모두 이용 불가],
)
...
#table(
  ...
  [합의 메커니즘], [2-Round 하이브리드 (Qwen 14B × 5+2)], [독립 투표 (Bedrock Claude Sonnet 4.6 × 3, 순차 실행 parallel=False). *만장일치 PASS* 요구; 하나라도 이견이면 WARN + minority_report 보존],
