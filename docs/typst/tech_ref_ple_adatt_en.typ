...
#align(center)[
  ...
  #block(width: 85%, stroke: (left: 2pt + anthropic-accent), inset: (left: 14pt, right: 14pt, top: 8pt, bottom: 8pt))[
    #text(size: 9.5pt, fill: anthropic-muted)[
      ...
      adaTT gradient 기반 태스크 전이, Task Groups 구조, 학습 전략,
...
== 1.1 Multi-Task Learning의 Motivation
...
+ *Expert Collapse 방지*: Shared/Task Expert Role이 자연스럽게 분리
...
본 구현에서는 두 가지 CGC 변형이 존재하며, Role과 출력 형태가 다르다.

=== CGCLayer (가중합 방식)

원본 PLE 논문의 CGC로, 태스크별 gate가 Shared + Task Expert 출력의 *가중합*을 산출한다.
Output Dimension은 `expert_hidden_dim`으로 고정된다 (Expert 수와 무관).
...
이종 Expert의 출력을 *연결(concatenation)*한 뒤 태스크별 attention weight로 각 Expert 블록을 스케일링한다. Output Dimension은 모든 Expert Output Dimension의 합이다.
...
각 Expert는 입력 데이터를 전혀 다른 수학적 Perspective으로 해석하며,
CGC Gate가 태스크별 최적 조합을 학습한다.

#styled-table(
  (1.3fr, 0.9fr, 0.6fr, 2.5fr),
  [*Expert*], [*입력*], [*출력*], [*Role 및 대체 불가능성*],
...
Negative Transfer가 발생한다. 고정 타워 MTL의 세 가지 Limitation:

+ *일방적 공유*: 태스크 간 간섭을 감지/조절할 메커니즘 부재
+ *상호작용 미측정*: 어떤 태스크 쌍이 돕고 해치는지 정량화 불가
+ *시간 변화 미반영*: 학습 Stage별 태스크 관계 변화를 추적 불가
...
#eq-highlight[
  ...
  #dim-label[
    ...
    신호처리 Perspective: IIR 1차 저역통과 필터 $H(z) = (1-alpha)/(1-alpha z^(-1))$
  ]
]
...
같은 Task Groups은 GroupEncoder를 공유하고, ClusterEmbedding으로 차별화한다:
...
#styled-table(
  (1fr, 0.6fr, 1fr, 2.5fr),
  [*태스크*], [*weight*], [*Focal $alpha$/$gamma$*], [*Notes*],
