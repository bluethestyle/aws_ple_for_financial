...
- EU AI Act 및 금융위 「금융분야 인공지능 가이드라인」(2026.6.22 시행)의 설명가능성 원칙에 정합하는 설명 가능한 추천을 지향한다
...
금융위 「금융분야 인공지능 가이드라인」 §4.4(신뢰성)는 전역(global)과 국소(local) 설명의 이원화를 권고하는데, 본 구조는 이에 직접 대응한다. gate weight 기반 expert 기여 분해가 전역 설명을 제공하며, 국소 설명은 예측 단위 IG attribution으로 보완한다(가이드라인은 자율 적용이며 권고 사항이다).
...
- 규제 관점(금융위 가이드라인 신뢰성·보조수단성 원칙): 모델이 "모른다"고 말할 수 있는 능력 = 안전장치
...
- expert 하나가 무력화돼도 gate 재분배로 지탱하는 graceful degradation은 금융위 가이드라인 §5.2(금융안정성)가 권고하는 백업모형(대체모형) 개념에 정합한다(자율 권고 사항)
