...
- **비즈니스 해석 불가**: 베이지안 업데이트 과정에서 각 모델의 기여가 혼합되면서,
  ...
  금융 규제 환경(금융위 「금융분야 인공지능 가이드라인」 설명가능성 원칙)에서 이는 치명적 약점.
...
| 제어이론 | Mamba (State Space) *(Temporal Ensemble 내부 구성 요소; 독립 expert 아님)* | 장기 행동 의존성 |
...
특히 2026.6.22 시행된 금융위 「금융분야 인공지능 가이드라인」 §4.4는 법적 설명의무가 있는 결정에는 SHAP 수준 이상의 설명가능성(XAI)을 권고하며, 본 아키텍처의 인과 설명 요소는 이에 정합한다(가이드라인은 자율 적용이며 권고 사항이다).
...
- ablation 23개 시나리오를 순차 실행하면 수일 소요
...
- ablation 병렬 실행 (Spot 4대 → 23시나리오를 ~4시간에)
...
- 23개 ablation 시나리오 설계 및 실행 완료 (14 joint + 9 structure)

### 진행 중
- 논문 최종 검수 및 제출 준비

### ~~남은 작업~~ (Historical TODO — all items completed as of v1 publication)
