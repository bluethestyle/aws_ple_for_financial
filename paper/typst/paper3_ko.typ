...
#set document(
  title: "동질 태스크를 넘어선 멀티태스크 학습 확장: 13-태스크 금융 추천에서의 손실 동역학과 게이트 선택",
...
#block(
  ...
  *인과 전문가 재해석* (Finding 8--13): 인과 전문가의 인접 행렬 $bold(W)$가
  ...
  중앙값 $32%$, 95번째 백분위수 $61%$로 상승함을 보여, Pearl의 Rung 3가
  증폭된 교사 위에서 성립 가능함을 입증한다. 그리고 인과 인코더 출력 타깃을
  ...
  *서빙 측 증류 제어* (Finding 14): 13-태스크 교사를 태스크별 gradient-boosted 학생으로 증류하면
...
*동반 논문 대비 위치.*
본 연구는 동일한 13개 태스크 벤치마크, 동일한 7개 전문가 PLE 백본,
동일한 v14 phase0 데이터 파이프라인을 공유하는 두 동반 논문 사이에 위치한다.
...
본 논문은 *Paper 1의 요약을 확장하여* (a) A--E 전반에 걸친 완전한
가설 식별(신호 정제, epoch 예산, 앙상블, 게이트 과적합,
정규화)과 13개 태스크 전체의 태스크별 v14
수치(Findings 1--6), (b) "CGC 위에서 어떤 종류의
...
우리 발견 중 두 가지의 출력 --- Causal Explainability Head가
생산하는 표본별 피처 attribution 벡터(Finding 9)와
Causal Guardrail이 생산하는 예측별 신뢰성 점수
(Findings 10--11) --- 를 가져와, 둘 모두를 GDPR 제22조의
의미 있는 설명과 EU AI Act 제13조의 투명성 요건을 충족하는
단일 HMAC 서명 해시 체인 예측별 감사 로그로 라우팅한다.
이 핸드셰이크는 구체적이다: 본 논문은 attribution 벡터와
coherence 점수를 생산하고, Paper 2는 둘 모두를 감사 로그에 배선하여
추천사유 생성 파이프라인을 통해 노출한다. Finding 11의
W-amplification 결과는 특히 감사 로그의 신뢰성 컬럼을 동기화한다:
기준 W 스케일에서는 CG가 제대로 식별하지 못하므로, Paper 2의 감사
표면은 amplified-W 교사 위에서만 실현 가능하다.
...
- Pearl Rung 3을 직접 시험하는 Counterfactual Probe(CCP, Finding 12):
  ...
  교사에서는 $0.16%$ 에 그친다 ---
  이는 $200 times$ 의 도약으로, amplified 교사 위에서*만*
  Rung 3 실현 가능성을 확립한다. 기준 체크포인트에서 DAG는
...
#figure(
  ...
  `configs/santander/pipeline.yaml` 의 `task_groups` 를 따른다 --- 본 논문에서
  보고하는 모든 실행에 사용된 13-태스크 구성(7 binary, 3 multiclass,
  3 regression)이다. 이후 누수 스캔에서 `segment_prediction` 이 라이브
  구성에서 제거되었으나, 본 논문의 모든 실행은 그 제거 이전 시점의 것이다.],
) <tab:taskgroups>
...
고객 100만 명, 13개 태스크(7 binary, 3 multiclass, 3 regression). 본 논문에서는
...
#figure(
  ...
  30 epoch). 낮은 엔트로피($E_t < 0.45$)는 1--2 개 전문가가 지배함을 나타내고,
...
Finding 2에서 loss-level `adaTT` 변형이 13-태스크 규모에서 집계 AUC에
...
13-태스크 벤치마크 결과는 9개 시나리오 전부에 대해 v14 SageMaker
...
- *게이트 포화 태스크*(segment_prediction, top_mcc_shift,
...
나머지 8개 태스크는 노이즈 범위 내에 있다($abs(Delta) <= 0.005$).
...
태스크별 상승은 13개 중 11개 태스크에 퍼진다(7개 binary 중 6개가 $+$0.0004 ~
...
- 배제(Rules out): 현재 타겟 설계(causal-encoder 출력 합의
  grad $times$ input)는 규제기관이 활용 가능한 샘플별 설명을
  산출하기에 *충분하지 않다*. 샘플별 attribution 품질에 의존하는
...
런타임 메커니즘을 요구하며, 인과 전문가에서의 자연스러운 hook 은
그 DAG 구조다.
...
== Finding 12: Counterfactual Probe (CCP) --- Pearl Rung 3 실행 가능성 <find12>
...
#figure(
  ...
  학습된 DAG가 반사실 질의를 수행할 수 있음을 확립한다.],
)<tbl:ccp-mediation>
...
서빙 스택은 13-task PLE 교사를 태스크별 LightGBM 학생으로
...
구성은 deterministic 태스크를 라벨 누출 위험으로 제거한 18-태스크
...
본 논문은 동일 프로젝트의 두 동반 논문을 보완한다. *논문 1*(아키텍처와
ablation)은 이질 expert PLE 설계를 확립하고 joint feature+expert
ablation 으로 expert 특화를 검증한다. *논문 2*(서빙과 운영)는 지식
증류, 추천사유 생성, 규제 준수를 다룬다. 본 논문은 ablation 연구 중에
부상했으나 논문 1 의 범위가 허용하는 것보다 깊은 분석을 요했던
*손실 동역학과 게이팅 거동*(발견 1--6), *fusion 증강 trade-off*(발견
7), *causal expert 재해석*(발견 8--13), *서빙 측 증류 제어*(발견 14)에
구체적으로 초점을 맞춘다.

발견 9--11 은 논문 2 의 v2 감사 인프라에 직접 소비된다. CEH
attribution 벡터는 `AuditLogger.log_attribution` 에, CG z-space
coherence score 는 `AuditLogger.log_guardrail` 에 유입되어, 완전한
HMAC 서명 해시 체인 per-prediction 레코드(설명 + 신뢰성)를 생성한다.
DAG 를 학습 가능하게 만드는 학습 시점 기제(발견 8)와 $bold(W)$ 를
구조적으로 의미 있게 만드는 증폭 노브(발견 11)는 따라서 논문 3 에서
독립적 호기심거리가 아니라, 논문 2 v2 에서 전달되는 규제기관 사용
가능 감사 표면의 전제조건이다.
...
붕괴하여, expert 를 평범한 MLP 와 동등하게 만들었다. 2부 패치(NOTEARS
reconstruction loss + 초기화 재스케일)는 primary-task 비용 없이 DAG
학습을 복원하지만, DAG 는 처음에는 "장식적"이다 --- 구조적으로
유효하나 예측으로 라우팅되지 않는다. 우리는 기능적 DAG 를 소비 가능한
per-prediction 출력으로 라우팅하는 두 Axis-3 후보를 보고한다. sample
단위 attribution 벡터를 산출하는 Causal Explainability Head(CEH),
그리고 신뢰성 플래그를 산출하는 Causal Guardrail(CG)이다. CEH 의 첫
정식화는 전역 중요도 패턴으로 붕괴했으며, 최소 개입 "demeaned target"
변형이 sample 단위 변별력을 복원했다. CG 의 첫 정식화는 우연 수준
변별에서 실패했으며, z-space Mahalanobis 정식화가 세 합성 OOD probe
에서 $5%$ FPR 에서 $100%$ TPR 을 달성했다. 이어 W-증폭 실험은 장식적
DAG 가 아키텍처적 제약이 아니라 학습 선택의 산물임을 확립했다 --- init
$0.1 arrow.r 0.3$ 과 $lambda_"recon" 0.5 arrow.r 2.0$ 이
$||bold(W)||_F$ 를 태스크 비용 없이 14 배로 키운다. 증폭된 teacher 를
기반으로, counterfactual probe(CCP)는 학습된 DAG 가 $"do"(z_j = v)$
개입 하에서 counterfactual 효과의 중앙값 $32%$, 95 백분위수 $61%$ 를
운반함을 측정하며, 이는 baseline teacher 의 $0.16%$ 와 대비된다 ---
추가 SageMaker 비용 없이 Pearl Rung 3 실현 가능성을 확립한다. 동반
논문에 기술된 HMAC 서명 해시 체인 감사 추적과 결합하면, CEH + CG 는
모델이 *무엇을* 추천했는가(attribution)와 그 추천을 *신뢰할 수
있는가*(신뢰성)를 짝짓는 규제기관 사용 가능 per-prediction 레코드를
생성한다.

*서빙 측 증류 제어*(발견 14): 13-태스크 teacher 를 태스크별
